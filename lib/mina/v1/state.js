...
exports.getLayout = exports.cleanStatePrecondition = exports.assertStatePrecondition = exports.declareState = exports.state = exports.State = void 0;
...
const wrapped_js_2 = require("../../provable/wrapped.js");
 ...
 */
function state(type) {
    let stateType = provable_intf_js_1.ProvableType.get(type);
...
exports.declareState = declareState;
    ...
    return {
        ...
        set(state) {
            ...
            stateAsFields.forEach((x, i) => {
                ...
                account_update_js_1.AccountUpdate.setValue(appStateSlot, x);
            });
        },
        requireEquals(state) {
            if (this._contract === undefined)
            ...
            stateAsFields.forEach((x, i) => {
                ...
                (0, precondition_js_1.ensureConsistentPrecondition)(precondition, (0, wrapped_js_2.Bool)(true), x, this._contract?.key);
                account_update_js_1.AccountUpdate.assertEquals(precondition, x);
            });
        ...
        requireEqualsIf(condition, state) {
            ...
            stateAsFields.forEach((stateField, i) => {
                let value = provable_js_1.Provable.if(condition, stateField, (0, wrapped_js_1.Field)(0));
                (0, precondition_js_1.ensureConsistentPrecondition)(accountUpdate.body.preconditions.account.state[layout.offset + i], condition, value, this._contract?.key);
        ...
        get() {
            ...
            if (this._contract.cachedVariable !== undefined &&
                ...
                provable_js_1.Provable.inCheckedComputation()) {
                this._contract.wasRead = true;
                return this._contract.cachedVariable;
            }
            let layout = getLayoutPosition(this._contract);
            let contract = this._contract;
            let inProver_ = provable_js_1.Provable.inProver();
            let stateFieldsType = provable_js_1.Provable.Array(wrapped_js_1.Field, layout.length);
            let stateAsFields = provable_js_1.Provable.witness(stateFieldsType, () => {
                let account;
                try {
                ...
                if (account.zkapp?.appState === undefined) {
                    ...
                    return Array(layout.length).fill((0, wrapped_js_1.Field)(0));
                }
                else {
                    let stateAsFields = [];
        ...
        async fetch() {
            ...
            let account;
            if (fetch_js_1.networkConfig.minaEndpoint === '') {
                account = Mina.getAccount(address, tokenId);
            }
            else {
                ({ account } = await (0, fetch_js_1.fetchAccount)({
                    publicKey: address,
                    tokenId: account_update_js_1.TokenId.toBase58(tokenId),
                }));
            }
            ...
            let stateAsFields;
            if (account.zkapp?.appState === undefined) {
                stateAsFields = Array(layout.length).fill((0, wrapped_js_1.Field)(0));
            }
            else {
                stateAsFields = [];
                for (let i = 0; i < layout.length; i++) {
...
exports.assertStatePrecondition = assertStatePrecondition;
...
exports.cleanStatePrecondition = cleanStatePrecondition;
