...
exports.invalidTransactionError = exports.humanizeErrors = void 0;
...
const ErrorHandlers = {
    Invalid_fee_excess({ transaction: { accountUpdates }, isFeePayer, accountCreationFee, }) {
        ...
        let balances = accountUpdates.map(({ body }) => {
            if (body.tokenId.equals(account_update_js_1.TokenId.default).toBoolean()) {
...
${balances
            .map((balance, i) => {
            return `Account update #${i + 1}) ${balance === undefined ? 'not a MINA account' : `${balance.toFixed(2)} MINA`}`;
        })
            .join(`\n`)}
