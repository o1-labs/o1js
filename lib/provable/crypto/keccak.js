...
const Keccak = {
    ...
    nistSha3(len, message) {
        return nistSha3(len, wrapped_classes_js_1.Bytes.from(message));
    },
    /**
     ...
     */
    ethereum(message) {
        return ethereum(wrapped_classes_js_1.Bytes.from(message));
    },
    /**
     ...
     */
    preNist(len, message) {
        return preNist(len, wrapped_classes_js_1.Bytes.from(message));
    },
};
...
function pad(message, rate, nist) {
    ...
    const pad = Array(extraBytes).fill(int_js_1.UInt8.from(0));
    pad[0] = int_js_1.UInt8.from(first);
...
const theta = (state) => {
    ...
    const stateD = Array.from({ length: KECCAK_DIM }, (_, i) => xor(stateC[(i + KECCAK_DIM - 1) % KECCAK_DIM], gadgets_js_1.Gadgets.rotate64(stateC[(i + 1) % KECCAK_DIM], 1, 'left')));
...
function piRho(state) {
    ...
    for (let i = 0; i < KECCAK_DIM; i++) {
        for (let j = 0; j < KECCAK_DIM; j++) {
            stateB[j][(2 * i + 3 * j) % KECCAK_DIM] = gadgets_js_1.Gadgets.rotate64(stateE[i][j], ROT_TABLE[i][j], 'left');
        }
    }
    return stateB;
}
...
function chi(state) {
    ...
    for (let i = 0; i < KECCAK_DIM; i++) {
        for (let j = 0; j < KECCAK_DIM; j++) {
            stateF[i][j] = xor(stateB[i][j], gadgets_js_1.Gadgets.and(
            ...
            gadgets_js_1.Gadgets.not(stateB[(i + 1) % KECCAK_DIM][j], KECCAK_WORD, false), stateB[(i + 2) % KECCAK_DIM][j], KECCAK_WORD));
        }
    }
    return stateF;
}
...
function iota(state, rc) {
    const stateG = state;
    stateG[0][0] = xor(stateG[0][0], field_js_1.Field.from(rc));
    return stateG;
}
...
function absorb(paddedMessage, capacity, rate, rc) {
    (0, errors_js_1.assert)(rate + capacity === KECCAK_STATE_LENGTH_WORDS, `invalid rate or capacity (rate + capacity should be ${KECCAK_STATE_LENGTH_WORDS})`);
    (0, errors_js_1.assert)(paddedMessage.length % rate === 0, 'invalid padded message length (should be multiple of rate)');
    let state = State.zeros();
    // array of capacity zero words
    const zeros = Array(capacity).fill(field_js_1.Field.from(0));
...
function squeeze(state, length, rate) {
    ...
    (0, errors_js_1.assert)(squeezes === 1, 'squeezes should be 1');
...
function sponge(paddedMessage, length, capacity, rate) {
    ...
    (0, errors_js_1.assert)(paddedMessage.length % rate === 0, 'Invalid padded message length');
    // absorb
...
function hash(message, length, capacity, nistVersion) {
    ...
    (0, errors_js_1.assert)(capacity > 0, 'capacity must be positive');
    (0, errors_js_1.assert)(capacity < KECCAK_STATE_LENGTH_BYTES, `capacity must be less than ${KECCAK_STATE_LENGTH_BYTES}`);
    (0, errors_js_1.assert)(length > 0, 'length must be positive');
    // convert capacity and length to word units
    (0, errors_js_1.assert)(capacity % BYTES_PER_WORD === 0, 'length must be a multiple of 8');
    capacity /= BYTES_PER_WORD;
    (0, errors_js_1.assert)(length % BYTES_PER_WORD === 0, 'length must be a multiple of 8');
    ...
    const padded = (0, bit_slices_js_1.bytesToWords)(paddedBytes);
    const hash = sponge(padded, length, capacity, rate);
    const hashBytes = (0, bit_slices_js_1.wordsToBytes)(hash);
    return hashBytes;
}
...
const State = {
     ...
     */
    zeros() {
        return Array.from(Array(KECCAK_DIM), (_) => Array(KECCAK_DIM).fill(field_js_1.Field.from(0)));
    },
    /**
     ...
     */
    xor(a, b) {
        (0, errors_js_1.assert)(a.length === KECCAK_DIM && a[0].length === KECCAK_DIM, `invalid \`a\` dimensions (should be ${KECCAK_DIM})`);
        (0, errors_js_1.assert)(b.length === KECCAK_DIM && b[0].length === KECCAK_DIM, `invalid \`b\` dimensions (should be ${KECCAK_DIM})`);
...
// AUXILIARY TYPES
class Bytes32 extends (0, wrapped_classes_js_1.Bytes)(32) {
}
class Bytes48 extends (0, wrapped_classes_js_1.Bytes)(48) {
}
class Bytes64 extends (0, wrapped_classes_js_1.Bytes)(64) {
}
...
function xor(x, y) {
    ...
    if (y.isConstant() && y.toBigInt() === 0n)
        return x;
    return gadgets_js_1.Gadgets.xor(x, y, 64);
}
