...
exports.getBlindingValue = exports.memoizeWitness = exports.memoizationContext = exports.Provable = void 0;
/**
 * {@link Provable} is
...
const Provable = {
     ...
     * ```
     */
    asProver: provable_context_js_1.asProver,
    /**
     ...
     */
    async runAndCheck(f) {
        await (0, provable_context_js_1.generateWitness)(f, { checkConstraints: true });
    },
    /**
    ...
    async runUnchecked(f) {
        await (0, provable_context_js_1.generateWitness)(f, { checkConstraints: false });
    },
    /**
     ...
     * ```
     */
    constraintSystem: provable_context_js_1.constraintSystem,
    /**
     ...
     * ```
     */
    inProver: provable_context_js_1.inProver,
    /**
     ...
     * ```
     */
    inCheckedComputation: provable_context_js_1.inCheckedComputation,
    /**
...
function equal(type, x, y) {
    ...
    return xs.map((x, i) => x.equals(ys[i])).reduce(bool_js_1.Bool.and);
}
...
function ifField(b, x, y) {
    ...
    if (typeof bindings_js_1.getCurrentBackend === 'function') {
        try {
            const currentBackend = (0, bindings_js_1.getCurrentBackend)();
            if (currentBackend === 'sparky' &&
                ...
                if (semanticResult) {
                    ...
                    return new field_js_1.Field(semanticResult);
                }
            ...
            console.debug('Sparky semantic If constraint failed, using primitive expansion:', error instanceof Error ? error.message : String(error));
        }
    }
...
function switch_(mask, type, values, { allowNonExclusive = false } = {}) {
    ...
    let fields = Array(size).fill(new field_js_1.Field(0));
...
function isConstant(type, x) {
    return provable_intf_js_1.ProvableType.get(type)
        .toFields(x)
        .every((x) => x.isConstant());
}
// logging in provable code
function log(...args) {
    (0, provable_context_js_1.asProver)(() => {
        let prettyArgs = [];
        for (let arg of args) {
...
let memoizationContext = global_context_js_1.Context.create();
exports.memoizationContext = memoizationContext;
/**
...
exports.memoizeWitness = memoizeWitness;
function getBlindingValue() {
    if (!memoizationContext.has())
        return field_js_1.Field.random();
    let context = memoizationContext.get();
    if (context.blindingValue === undefined) {
        context.blindingValue = field_js_1.Field.random();
    }
    return context.blindingValue;
}
exports.getBlindingValue = getBlindingValue;
...
function provableArray(elementType, length) {
    let type = provable_intf_js_1.ProvableType.get(elementType);
    return {
        /**
        ...
        toInput(array) {
            ...
            return array.reduce((curr, value) => provable_derivers_js_1.HashInput.append(curr, type.toInput(value)), provable_derivers_js_1.HashInput.empty);
        },
        empty() {
            if (!('empty' in type)) {
