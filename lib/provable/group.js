...
class Group {
    ...
    static get generator() {
        return new Group({ x: elliptic_curve_js_1.Pallas.one.x, y: elliptic_curve_js_1.Pallas.one.y });
    }
    /**
    ...
    constructor({ x, y, }) {
        ...
        if (isConstant(this)) {
            ...
            if (this.x.equals(0).and(this.y.equals(0)).toBoolean())
                return;
            const { add, mul, square } = finite_field_js_1.Fp;
            let x_bigint = this.x.toBigInt();
            let y_bigint = this.y.toBigInt();
            let onCurve = add(mul(x_bigint, mul(x_bigint, x_bigint)), elliptic_curve_js_1.Pallas.b) === square(y_bigint);
            if (!onCurve) {
    ...
    add(g) {
        if (isConstant(this) && isConstant(g)) {
            ...
            if (this.isZero().toBoolean()) {
                ...
                let g_proj = elliptic_curve_js_1.Pallas.add(toProjective(this), toProjective(g));
                return fromProjective(g_proj);
            }
        }
        else {
            let { result, isInfinity } = (0, native_curve_js_1.add)(this, g);
            ...
            return provable_js_1.Provable.switch([gIsZero, onlyThisIsZero, isNegation, isNormalAddition], Group, [this, g, Group.zero, new Group(result)], { allowNonExclusive: true });
        }
    }
    /**
    ...
    addNonZero(g2, allowZeroOutput = false) {
        if (isConstant(this) && isConstant(g2)) {
            let { x, y, infinity } = elliptic_curve_js_1.PallasAffine.add(toAffine(this), toAffine(g2));
            (0, assert_js_1.assert)(!infinity || allowZeroOutput, 'Group.addNonzero(): Result is zero');
            return fromAffine({ x, y, infinity });
        }
        let { result, isInfinity } = (0, native_curve_js_1.add)(this, g2);
        if (allowZeroOutput) {
            return provable_js_1.Provable.if(isInfinity, Group.zero, new Group(result));
        }
        else {
     ...
     */
    scale(s) {
        if (s instanceof field_js_1.Field)
            return new Group((0, native_curve_js_1.scaleField)(this, s));
        let scalar = scalar_js_1.Scalar.from(s);
        if (isConstant(this) && scalar.isConstant()) {
            let g_proj = elliptic_curve_js_1.Pallas.scale(toProjective(this), scalar.toBigInt());
            return fromProjective(g_proj);
        }
        else {
            let result = (0, native_curve_js_1.scaleShifted)(this, scalar);
            return new Group(result);
        }
    }
    /**
    ...
    static check(g) {
        try {
            ...
            let x2 = x.square();
            let x3 = x2.mul(x);
            let ax = x.mul(elliptic_curve_js_1.Pallas.a); // this will obviously be 0, but just for the sake of correctness
            // we also check the zero element (0, 0) here
            let isZero = x.equals(0).and(y.equals(0));
            isZero.or(x3.add(ax).add(elliptic_curve_js_1.Pallas.b).equals(y.square())).assertTrue();
        }
        catch (error) {
...
function toProjective(g) {
    return elliptic_curve_js_1.Pallas.fromAffine({
        x: g.x.toBigInt(),
        y: g.y.toBigInt(),
        infinity: false,
    });
...
function fromProjective({ x, y, z }) {
    return fromAffine(elliptic_curve_js_1.Pallas.toAffine({ x, y, z }));
}
function fromAffine({ x, y, infinity }) {
    return infinity ? Group.zero : new Group({ x, y });
}
function toAffine(g) {
    return elliptic_curve_js_1.PallasAffine.from({ x: g.x.toBigInt(), y: g.y.toBigInt() });
}
