"use strict";
var __classPrivateFieldSet = (this && this.__classPrivateFieldSet) || function (receiver, state, value, kind, f) {
    if (kind === "m") throw new TypeError("Private method is not writable");
    if (kind === "a" && !f) throw new TypeError("Private accessor was defined without a setter");
    if (typeof state === "function" ? receiver !== state || !f : !state.has(receiver)) throw new TypeError("Cannot write private member to an object whose class did not declare it");
    return (kind === "a" ? f.call(receiver, value) : f ? f.value = value : state.set(receiver, value)), value;
};
var __classPrivateFieldGet = (this && this.__classPrivateFieldGet) || function (receiver, state, kind, f) {
    if (kind === "a" && !f) throw new TypeError("Private accessor was defined without a getter");
    if (typeof state === "function" ? receiver !== state || !f : !state.has(receiver)) throw new TypeError("Cannot read private member from an object whose class did not declare it");
    return kind === "m" ? f : kind === "a" ? f.call(receiver) : f ? f.value : state.get(receiver);
};
var _Sum_instances, _Sum_result, _Sum_summands, _Sum_ops, _Sum_return;
Object.defineProperty(exports, "__esModule", { value: true });
exports.field3FromBits = exports.assertMul = exports.Sum = exports.weakBound = exports.combine = exports.split = exports.Field3 = exports.ForeignField = void 0;
/**
 * Foreign field arithmetic gadgets.
 */
const finite_field_js_1 = require("../../../bindings/crypto/finite-field.js");
const provable_derivers_js_1 = require("../types/provable-derivers.js");
const unconstrained_js_1 = require("../types/unconstrained.js");
const gates_js_1 = require("../gates.js");
const exists_js_1 = require("../core/exists.js");
const fields_js_1 = require("../types/fields.js");
const types_js_1 = require("../../util/types.js");
const basic_js_1 = require("./basic.js");
const common_js_1 = require("./common.js");
const range_check_js_1 = require("./range-check.js");
const field_constructor_js_1 = require("../core/field-constructor.js");
const ForeignField = {
    add(x, y, f) {
        return sum([x, y], [1n], f);
    },
    sub(x, y, f) {
        return sum([x, y], [-1n], f);
    },
    negate(x, f) {
        return sum([Field3.from(0n), x], [-1n], f);
    },
    sum,
    Sum(x) {
        return new Sum(x);
    },
    mul: multiply,
    inv: inverse,
    div: divide,
    assertMul,
    assertAlmostReduced,
    assertLessThan,
    assertLessThanOrEqual,
    assertEquals,
    equals,
    toCanonical,
};
exports.ForeignField = ForeignField;
/**
 * computes x[0] + sign[0] * x[1] + ... + sign[n-1] * x[n] modulo f
 *
 * assumes that inputs are range checked, does range check on the result.
 */
function sum(x, sign, f) {
    (0, common_js_1.assert)(x.length === sign.length + 1, 'inputs and operators match');
    // constant case
    if (x.every(Field3.isConstant)) {
        let xBig = x.map(Field3.toBigint);
        let sum = sign.reduce((sum, s, i) => sum + s * xBig[i + 1], xBig[0]);
        return Field3.from((0, finite_field_js_1.mod)(sum, f));
    }
    // provable case - create chain of ffadd rows
    x = x.map(common_js_1.toVars);
    let result = x[0];
    for (let i = 0; i < sign.length; i++) {
        ({ result } = singleAdd(result, x[i + 1], sign[i], f));
    }
    // final zero row to hold result
    gates_js_1.Gates.zero(...result);
    // range check result
    indirectMultiRangeChange(result);
    return result;
}
/**
 * core building block for non-native addition
 *
 * **warning**: this just adds the `foreignFieldAdd` row;
 * it _must_ be chained with a second row that holds the result in its first 3 cells.
 *
 * the second row could, for example, be `zero`, `foreignFieldMul`, or another `foreignFieldAdd`.
 */
function singleAdd(x, y, sign, f) {
    let f_ = split(f);
    let [r0, r1, r2, overflow, carry] = (0, exists_js_1.exists)(5, () => {
        let x_ = toBigint3(x);
        let y_ = toBigint3(y);
        // figure out if there's overflow
        let r = combine(x_) + sign * combine(y_);
        let overflow = 0n;
        if (sign === 1n && r >= f)
            overflow = 1n;
        if (sign === -1n && r < 0n)
            overflow = -1n;
        if (f === 0n)
            overflow = 0n; // special case where overflow doesn't change anything
        // do the add with carry
        // note: this "just works" with negative r01
        let r01 = combine2(x_) + sign * combine2(y_) - overflow * combine2(f_);
        let carry = r01 >> range_check_js_1.l2;
        r01 &= range_check_js_1.l2Mask;
        let [r0, r1] = split2(r01);
        let r2 = x_[2] + sign * y_[2] - overflow * f_[2] + carry;
        return [r0, r1, r2, overflow, carry];
    });
    (0, gates_js_1.foreignFieldAdd)({ left: x, right: y, overflow, carry, modulus: f_, sign });
    return { result: [r0, r1, r2], overflow };
}
function multiply(a, b, f) {
    (0, common_js_1.assert)(f < 1n << 259n, 'Foreign modulus fits in 259 bits');
    // constant case
    if (Field3.isConstant(a) && Field3.isConstant(b)) {
        let ab = Field3.toBigint(a) * Field3.toBigint(b);
        return Field3.from((0, finite_field_js_1.mod)(ab, f));
    }
    // provable case
    let { r01, r2, q } = multiplyNoRangeCheck(a, b, f);
    // limb range checks on quotient and remainder
    (0, range_check_js_1.multiRangeCheck)(q);
    let r = (0, range_check_js_1.compactMultiRangeCheck)(r01, r2);
    return r;
}
function inverse(x, f) {
    (0, common_js_1.assert)(f < 1n << 259n, 'Foreign modulus fits in 259 bits');
    // constant case
    if (Field3.isConstant(x)) {
        let xInv = (0, finite_field_js_1.inverse)(Field3.toBigint(x), f);
        (0, common_js_1.assert)(xInv !== undefined, 'inverse exists');
        return Field3.from(xInv);
    }
    // provable case
    let xInv = (0, exists_js_1.exists)(3, () => {
        let xInv = (0, finite_field_js_1.inverse)(Field3.toBigint(x), f);
        return xInv === undefined ? [0n, 0n, 0n] : split(xInv);
    });
    (0, range_check_js_1.multiRangeCheck)(xInv);
    // we need to bound xInv because it's a multiplication input
    let xInv2Bound = weakBound(xInv[2], f);
    let one = [(0, field_constructor_js_1.createField)(1n), (0, field_constructor_js_1.createField)(0n)];
    assertMulInternal(x, xInv, one, f);
    // range check on result bound
    // TODO: this uses two RCs too many.. need global RC stack
    (0, range_check_js_1.multiRangeCheck)([xInv2Bound, (0, field_constructor_js_1.createField)(0n), (0, field_constructor_js_1.createField)(0n)]);
    return xInv;
}
function divide(x, y, f, { allowZeroOverZero = false } = {}) {
    (0, common_js_1.assert)(f < 1n << 259n, 'Foreign modulus fits in 259 bits');
    // constant case
    if (Field3.isConstant(x) && Field3.isConstant(y)) {
        let yInv = (0, finite_field_js_1.inverse)(Field3.toBigint(y), f);
        (0, common_js_1.assert)(yInv !== undefined, 'inverse exists');
        return Field3.from((0, finite_field_js_1.mod)(Field3.toBigint(x) * yInv, f));
    }
    // provable case
    // to show that z = x/y, we prove that z*y = x and y != 0 (the latter avoids the unconstrained 0/0 case)
    let z = (0, exists_js_1.exists)(3, () => {
        let yInv = (0, finite_field_js_1.inverse)(Field3.toBigint(y), f);
        if (yInv === undefined)
            return [0n, 0n, 0n];
        return split((0, finite_field_js_1.mod)(Field3.toBigint(x) * yInv, f));
    });
    (0, range_check_js_1.multiRangeCheck)(z);
    let z2Bound = weakBound(z[2], f);
    assertMulInternal(z, y, x, f);
    // range check on result bound
    (0, range_check_js_1.multiRangeCheck)([z2Bound, (0, field_constructor_js_1.createField)(0n), (0, field_constructor_js_1.createField)(0n)]);
    if (!allowZeroOverZero) {
        ForeignField.equals(y, 0n, f).assertFalse();
    }
    return z;
}
/**
 * Common logic for gadgets that expect a certain multiplication result a priori, instead of just using the remainder.
 */
function assertMulInternal(x, y, xy, f, message) {
    let { r01, r2, q } = multiplyNoRangeCheck(x, y, f);
    // range check on quotient
    (0, range_check_js_1.multiRangeCheck)(q);
    // bind remainder to input xy
    if (xy.length === 2) {
        let [xy01, xy2] = xy;
        r01.assertEquals(xy01, message);
        r2.assertEquals(xy2, message);
    }
    else {
        let xy01 = xy[0].add(xy[1].mul(1n << range_check_js_1.l));
        r01.assertEquals(xy01, message);
        r2.assertEquals(xy[2], message);
    }
}
/**
 * Core building block for all gadgets using foreign field multiplication.
 */
function multiplyNoRangeCheck(a, b, f) {
    // notation follows https://github.com/o1-labs/rfcs/blob/main/0006-ffmul-revised.md
    let f_ = (1n << range_check_js_1.l3) - f;
    let [f_0, f_1, f_2] = split(f_);
    let f2 = f >> range_check_js_1.l2;
    let f2Bound = (1n << range_check_js_1.l) - f2 - 1n;
    let witnesses = (0, exists_js_1.exists)(21, () => {
        // convert inputs to bigints
        let [a0, a1, a2] = toBigint3(a);
        let [b0, b1, b2] = toBigint3(b);
        // compute q and r such that a*b = q*f + r
        let ab = combine([a0, a1, a2]) * combine([b0, b1, b2]);
        let q = ab / f;
        let r = ab - q * f;
        let [q0, q1, q2] = split(q);
        let [r0, r1, r2] = split(r);
        let r01 = combine2([r0, r1]);
        // compute product terms
        let p0 = a0 * b0 + q0 * f_0;
        let p1 = a0 * b1 + a1 * b0 + q0 * f_1 + q1 * f_0;
        let p2 = a0 * b2 + a1 * b1 + a2 * b0 + q0 * f_2 + q1 * f_1 + q2 * f_0;
        let [p10, p110, p111] = split(p1);
        let p11 = combine2([p110, p111]);
        // carry bottom limbs
        let c0 = (p0 + (p10 << range_check_js_1.l) - r01) >> range_check_js_1.l2;
        // carry top limb
        let c1 = (p2 - r2 + p11 + c0) >> range_check_js_1.l;
        // split high carry
        let c1_00 = (0, common_js_1.bitSlice)(c1, 0, 12);
        let c1_12 = (0, common_js_1.bitSlice)(c1, 12, 12);
        let c1_24 = (0, common_js_1.bitSlice)(c1, 24, 12);
        let c1_36 = (0, common_js_1.bitSlice)(c1, 36, 12);
        let c1_48 = (0, common_js_1.bitSlice)(c1, 48, 12);
        let c1_60 = (0, common_js_1.bitSlice)(c1, 60, 12);
        let c1_72 = (0, common_js_1.bitSlice)(c1, 72, 12);
        let c1_84 = (0, common_js_1.bitSlice)(c1, 84, 2);
        let c1_86 = (0, common_js_1.bitSlice)(c1, 86, 2);
        let c1_88 = (0, common_js_1.bitSlice)(c1, 88, 2);
        let c1_90 = (0, common_js_1.bitSlice)(c1, 90, 1);
        // quotient high bound
        let q2Bound = q2 + f2Bound;
        // prettier-ignore
        return [
            r01, r2,
            q0, q1, q2,
            q2Bound,
            p10, p110, p111,
            c0,
            c1_00, c1_12, c1_24, c1_36, c1_48, c1_60, c1_72,
            c1_84, c1_86, c1_88, c1_90,
        ];
    });
    // prettier-ignore
    let [r01, r2, q0, q1, q2, q2Bound, p10, p110, p111, c0, c1_00, c1_12, c1_24, c1_36, c1_48, c1_60, c1_72, c1_84, c1_86, c1_88, c1_90,] = witnesses;
    let q = [q0, q1, q2];
    // ffmul gate. this already adds the following zero row.
    gates_js_1.Gates.foreignFieldMul({
        left: a,
        right: b,
        remainder: [r01, r2],
        quotient: q,
        quotientHiBound: q2Bound,
        product1: [p10, p110, p111],
        carry0: c0,
        carry1p: [c1_00, c1_12, c1_24, c1_36, c1_48, c1_60, c1_72],
        carry1c: [c1_84, c1_86, c1_88, c1_90],
        foreignFieldModulus2: f2,
        negForeignFieldModulus: [f_0, f_1, f_2],
    });
    // multi-range check on internal values
    (0, range_check_js_1.multiRangeCheck)([p10, p110, q2Bound]);
    // note: this function is supposed to be the most flexible interface to the ffmul gate.
    // that's why we don't add range checks on q and r here, because there are valid use cases
    // for not range-checking either of them -- for example, they could be wired to other
    // variables that are already range-checked, or to constants / public inputs.
    return { r01, r2, q };
}
function weakBound(x, f) {
    // if f0, f1 === 0, we can use a stronger bound x[2] < f2
    // because this is true for all field elements x in [0,f)
    if ((f & range_check_js_1.l2Mask) === 0n) {
        return x.add(range_check_js_1.lMask + 1n - (f >> range_check_js_1.l2));
    }
    // otherwise, we use x[2] < f2 + 1, so we allow x[2] === f2
    return x.add(range_check_js_1.lMask - (f >> range_check_js_1.l2));
}
exports.weakBound = weakBound;
/**
 * Apply range checks and weak bounds checks to a list of Field3s.
 * Optimal if the list length is a multiple of 3.
 */
function assertAlmostReduced(xs, f, skipMrc = false) {
    let bounds = [];
    for (let x of xs) {
        if (!skipMrc)
            (0, range_check_js_1.multiRangeCheck)(x);
        bounds.push(weakBound(x[2], f));
        if (types_js_1.TupleN.hasLength(3, bounds)) {
            (0, range_check_js_1.multiRangeCheck)(bounds);
            bounds = [];
        }
    }
    if (types_js_1.TupleN.hasLength(1, bounds)) {
        (0, range_check_js_1.multiRangeCheck)([...bounds, (0, field_constructor_js_1.createField)(0n), (0, field_constructor_js_1.createField)(0n)]);
    }
    if (types_js_1.TupleN.hasLength(2, bounds)) {
        (0, range_check_js_1.multiRangeCheck)([...bounds, (0, field_constructor_js_1.createField)(0n)]);
    }
}
/**
 * check whether x = c mod f
 *
 * c is a constant, and we require c in [0, f)
 *
 * assumes that x is almost reduced mod f, so we know that x might be c or c + f, but not c + 2f, c + 3f, ...
 */
function equals(x, c, f) {
    (0, common_js_1.assert)(c >= 0n && c < f, 'equals: c must be in [0, f)');
    // constant case
    if (Field3.isConstant(x)) {
        return (0, field_constructor_js_1.createBool)((0, finite_field_js_1.mod)(Field3.toBigint(x), f) === c);
    }
    // provable case
    if (f >= 1n << range_check_js_1.l2) {
        // check whether x = 0 or x = f
        let x01 = (0, common_js_1.toVar)(x[0].add(x[1].mul(1n << range_check_js_1.l)));
        let [c01, c2] = [c & range_check_js_1.l2Mask, c >> range_check_js_1.l2];
        let [cPlusF01, cPlusF2] = [(c + f) & range_check_js_1.l2Mask, (c + f) >> range_check_js_1.l2];
        // (x01, x2) = (c01, c2)
        let isC = x01.equals(c01).and(x[2].equals(c2));
        // (x01, x2) = (cPlusF01, cPlusF2)
        let isCPlusF = x01.equals(cPlusF01).and(x[2].equals(cPlusF2));
        return isC.or(isCPlusF);
    }
    else {
        // if f < 2^2l, the approach above doesn't work (we don't know from x[2] = 0 that x < 2f),
        // so in that case we assert that x < f and then check whether it's equal to c
        ForeignField.assertLessThan(x, f);
        let x012 = (0, common_js_1.toVar)(x[0].add(x[1].mul(1n << range_check_js_1.l)).add(x[2].mul(1n << range_check_js_1.l2)));
        return x012.equals(c);
    }
}
// Field3 equality checking that each of the 3 limbs are exactly the same (not modular equality).
function assertEquals(x, y) {
    // constant case
    if (Field3.isConstant(x) && Field3.isConstant(y)) {
        (0, common_js_1.assert)(Field3.toBigint(x) === Field3.toBigint(y), 'assertEqual: got x != y');
        return;
    }
    //provable case
    x[0].assertEquals(y[0]);
    x[1].assertEquals(y[1]);
    x[2].assertEquals(y[2]);
}
/**
 * Convert x, which may be unreduced, to a canonical representative < f.
 *
 * Note: This method is complete, it works for all unreduced field elements.
 * It can therefore be used to protect against incompleteness of field operations in other places.
 */
function toCanonical(x, f) {
    // multiply by 1 to get reduced representative
    // note: this is sound because x < 2^3l << 2^3l p
    let xR = multiply(x, Field3.from(1n), f);
    // assert the result is canonical, and return it
    assertLessThan(xR, f);
    return xR;
}
const provableLimb = (0, fields_js_1.modifiedField)({});
const Field3 = {
    /**
     * Turn a bigint into a 3-tuple of Fields
     */
    from(x) {
        if (Array.isArray(x))
            return x;
        return types_js_1.Tuple.map(split(x), field_constructor_js_1.createField);
    },
    /**
     * Turn a 3-tuple of Fields into a bigint
     */
    toBigint(x) {
        return combine(toBigint3(x));
    },
    /**
     * Turn several 3-tuples of Fields into bigints
     */
    toBigints(...xs) {
        return types_js_1.Tuple.map(xs, Field3.toBigint);
    },
    /**
     * Check whether a 3-tuple of Fields is constant
     */
    isConstant(x) {
        return x.every((x) => x.isConstant());
    },
    /**
     * `Provable<T>` interface for `Field3 = [Field, Field, Field]`.
     *
     * Note: Witnessing this creates a plain tuple of field elements without any implicit
     * range checks.
     */
    provable: {
        ...(0, provable_derivers_js_1.provableTuple)([provableLimb, provableLimb, provableLimb]),
        toValue(x) {
            return Field3.toBigint(x);
        },
        fromValue(x) {
            if (typeof x === 'bigint')
                return Field3.from(x);
            return x;
        },
    },
    /**
     * Splits a bigint into three limbs using bitwise operations.
     */
    split,
};
exports.Field3 = Field3;
const Field2 = {
    toBigint(x) {
        return combine2(types_js_1.Tuple.map(x, (x) => x.toBigInt()));
    },
};
function toBigint3(x) {
    return types_js_1.Tuple.map(x, (x) => x.toBigInt());
}
function combine([x0, x1, x2]) {
    return x0 + (x1 << range_check_js_1.l) + (x2 << range_check_js_1.l2);
}
exports.combine = combine;
function split(x) {
    return [x & range_check_js_1.lMask, (x >> range_check_js_1.l) & range_check_js_1.lMask, (x >> range_check_js_1.l2) & range_check_js_1.lMask];
}
exports.split = split;
function combine2([x0, x1]) {
    return x0 + (x1 << range_check_js_1.l);
}
function split2(x) {
    return [x & range_check_js_1.lMask, (x >> range_check_js_1.l) & range_check_js_1.lMask];
}
/**
 * Optimized multiplication of sums, like (x + y)*z = a + b + c
 *
 * We use several optimizations over naive summing and then multiplying:
 *
 * - we skip the range check on the remainder sum, because ffmul is sound with r being a sum of range-checked values
 * - we replace the range check on the input sums with an extra low limb sum using generic gates
 * - we chain the first input's sum into the ffmul gate
 *
 * As usual, all values are assumed to be range checked, and the left and right multiplication inputs
 * are assumed to be bounded such that `l * r < 2^264 * (native modulus)`.
 * However, all extra checks that are needed on the _sums_ are handled here.
 */
function assertMul(x, y, xy, f, message) {
    x = Sum.fromUnfinished(x);
    y = Sum.fromUnfinished(y);
    xy = Sum.fromUnfinished(xy);
    // conservative estimate to ensure that multiplication bound is satisfied
    // we assume that all summands si are bounded with si[2] <= f[2] checks, which implies si < 2^k where k := ceil(log(f))
    // our assertion below gives us
    // |x|*|y| + q*f + |r| < (x.length * y.length) 2^2k + 2^2k + 2^2k < 3 * 2^(2*258) < 2^264 * (native modulus)
    (0, common_js_1.assert)(BigInt(Math.ceil(Math.sqrt(x.length * y.length))) * f < 1n << 258n, `Foreign modulus is too large for multiplication of sums of lengths ${x.length} and ${y.length}`);
    // finish the y and xy sums with a zero gate
    let y0 = y.finishForMulInput(f);
    let xy0 = xy.finish(f);
    // x is chained into the ffmul gate
    let x0 = x.finishForMulInput(f, true);
    // constant case
    if (Field3.isConstant(x0) && Field3.isConstant(y0) && Field3.isConstant(xy0)) {
        let x_ = Field3.toBigint(x0);
        let y_ = Field3.toBigint(y0);
        let xy_ = Field3.toBigint(xy0);
        (0, common_js_1.assert)((0, finite_field_js_1.mod)(x_ * y_, f) === xy_, message ?? 'assertMul(): incorrect multiplication result');
        return;
    }
    assertMulInternal(x0, y0, xy0, f, message);
}
exports.assertMul = assertMul;
/**
 * @internal
 *
 * Lazy sum of {@link Field3} elements, which can be used as input to `Gadgets.ForeignField.assertMul()`.
 */
class Sum {
    constructor(x) {
        _Sum_instances.add(this);
        _Sum_result.set(this, void 0);
        _Sum_summands.set(this, void 0);
        _Sum_ops.set(this, []);
        __classPrivateFieldSet(this, _Sum_summands, [x], "f");
    }
    get result() {
        (0, common_js_1.assert)(__classPrivateFieldGet(this, _Sum_result, "f") !== undefined, 'sum not finished');
        return __classPrivateFieldGet(this, _Sum_result, "f");
    }
    get length() {
        return __classPrivateFieldGet(this, _Sum_summands, "f").length;
    }
    add(y) {
        (0, common_js_1.assert)(__classPrivateFieldGet(this, _Sum_result, "f") === undefined, 'sum already finished');
        __classPrivateFieldGet(this, _Sum_ops, "f").push(1n);
        __classPrivateFieldGet(this, _Sum_summands, "f").push(y);
        return this;
    }
    sub(y) {
        (0, common_js_1.assert)(__classPrivateFieldGet(this, _Sum_result, "f") === undefined, 'sum already finished');
        __classPrivateFieldGet(this, _Sum_ops, "f").push(-1n);
        __classPrivateFieldGet(this, _Sum_summands, "f").push(y);
        return this;
    }
    isConstant() {
        return __classPrivateFieldGet(this, _Sum_summands, "f").every(Field3.isConstant);
    }
    finish(f, isChained = false) {
        (0, common_js_1.assert)(__classPrivateFieldGet(this, _Sum_result, "f") === undefined, 'sum already finished');
        let signs = __classPrivateFieldGet(this, _Sum_ops, "f");
        let n = signs.length;
        if (n === 0)
            return __classPrivateFieldGet(this, _Sum_instances, "m", _Sum_return).call(this, __classPrivateFieldGet(this, _Sum_summands, "f")[0]);
        // constant case
        if (this.isConstant()) {
            return __classPrivateFieldGet(this, _Sum_instances, "m", _Sum_return).call(this, sum(__classPrivateFieldGet(this, _Sum_summands, "f"), signs, f));
        }
        // provable case
        let x = __classPrivateFieldGet(this, _Sum_summands, "f").map(common_js_1.toVars);
        let result = x[0];
        for (let i = 0; i < n; i++) {
            ({ result } = singleAdd(result, x[i + 1], signs[i], f));
        }
        if (!isChained)
            gates_js_1.Gates.zero(...result);
        __classPrivateFieldSet(this, _Sum_result, result, "f");
        return result;
    }
    // TODO this is complex and should be removed once we fix the ffadd gate to constrain all limbs individually
    finishForMulInput(f, isChained = false) {
        (0, common_js_1.assert)(__classPrivateFieldGet(this, _Sum_result, "f") === undefined, 'sum already finished');
        let signs = __classPrivateFieldGet(this, _Sum_ops, "f");
        let n = signs.length;
        if (n === 0)
            return __classPrivateFieldGet(this, _Sum_instances, "m", _Sum_return).call(this, __classPrivateFieldGet(this, _Sum_summands, "f")[0]);
        // constant case
        if (this.isConstant()) {
            return __classPrivateFieldGet(this, _Sum_instances, "m", _Sum_return).call(this, sum(__classPrivateFieldGet(this, _Sum_summands, "f"), signs, f));
        }
        // provable case
        let xs = __classPrivateFieldGet(this, _Sum_summands, "f").map(common_js_1.toVars);
        // since the sum becomes a multiplication input, we need to constrain all limbs _individually_.
        // sadly, ffadd only constrains the low and middle limb together.
        // we could fix it with a RC just for the lower two limbs
        // but it's cheaper to add generic gates which handle the lowest limb separately, and avoids the unfilled MRC slot
        let f0 = f & range_check_js_1.lMask;
        // generic gates for low limbs
        let x0 = xs[0][0];
        let x0s = [];
        let overflows = [];
        let xRef = unconstrained_js_1.Unconstrained.witness(() => Field3.toBigint(xs[0]));
        // this loop mirrors the computation that a chain of ffadd gates does,
        // but everything is done only on the lowest limb and using generic gates.
        // the output is a sequence of low limbs (x0) and overflows, which will be wired to the ffadd results at each step.
        for (let i = 0; i < n; i++) {
            // compute carry and overflow
            let [carry, overflow] = (0, exists_js_1.exists)(2, () => {
                // this duplicates some of the logic in singleAdd
                let x = xRef.get();
                let x0 = x & range_check_js_1.lMask;
                let xi = toBigint3(xs[i + 1]);
                let sign = signs[i];
                // figure out if there's overflow
                x += sign * combine(xi);
                let overflow = 0n;
                if (sign === 1n && x >= f)
                    overflow = 1n;
                if (sign === -1n && x < 0n)
                    overflow = -1n;
                if (f === 0n)
                    overflow = 0n;
                xRef.set(x - overflow * f);
                // add with carry, only on the lowest limb
                x0 = x0 + sign * xi[0] - overflow * f0;
                let carry = x0 >> range_check_js_1.l;
                return [carry, overflow];
            });
            overflows.push(overflow);
            // constrain carry
            (0, basic_js_1.assertOneOf)(carry, [0n, 1n, -1n]);
            // x0 <- x0 + s*xi0 - o*f0 - c*2^l
            x0 = (0, common_js_1.toVar)(x0
                .add(xs[i + 1][0].mul(signs[i]))
                .sub(overflow.mul(f0))
                .sub(carry.mul(1n << range_check_js_1.l)));
            x0s.push(x0);
        }
        // ffadd chain
        let x = xs[0];
        for (let i = 0; i < n; i++) {
            let { result, overflow } = singleAdd(x, xs[i + 1], signs[i], f);
            // wire low limb and overflow to previous values
            result[0].assertEquals(x0s[i]);
            overflow.assertEquals(overflows[i]);
            x = result;
        }
        if (!isChained)
            gates_js_1.Gates.zero(...x);
        __classPrivateFieldSet(this, _Sum_result, x, "f");
        return x;
    }
    rangeCheck() {
        (0, common_js_1.assert)(__classPrivateFieldGet(this, _Sum_result, "f") !== undefined, 'sum not finished');
        if (__classPrivateFieldGet(this, _Sum_ops, "f").length > 0)
            (0, range_check_js_1.multiRangeCheck)(__classPrivateFieldGet(this, _Sum_result, "f"));
    }
    static fromUnfinished(x) {
        if (x instanceof Sum) {
            (0, common_js_1.assert)(__classPrivateFieldGet(x, _Sum_result, "f") === undefined, 'sum already finished');
            return x;
        }
        return new Sum(x);
    }
}
exports.Sum = Sum;
_Sum_result = new WeakMap(), _Sum_summands = new WeakMap(), _Sum_ops = new WeakMap(), _Sum_instances = new WeakSet(), _Sum_return = function _Sum_return(x) {
    __classPrivateFieldSet(this, _Sum_result, x, "f");
    return x;
};
// Field3 comparison
function assertLessThan(x, y) {
    let y_ = Field3.from(y);
    // constant case, y = constant, x = constant
    if (Field3.isConstant(x) && Field3.isConstant(y_)) {
        (0, common_js_1.assert)(Field3.toBigint(x) < Field3.toBigint(y_), 'assertLessThan: got x >= y');
        return;
    }
    // case of y = constant, x = variable
    if (Field3.isConstant(y_)) {
        y = typeof y === 'bigint' ? y : Field3.toBigint(y);
        // this case is not included below, because ffadd doesn't support negative moduli
        (0, common_js_1.assert)(y > 0n, 'assertLessThan: y <= 0, so x < y is impossible');
        // we can just use negation `(y - 1) - x`. because the result is range-checked, it proves that x < y:
        // `y - 1 - x \in [0, 2^3l) => x <= x + (y - 1 - x) = y - 1 < y`
        // (note: ffadd can't add higher multiples of (f - 1). it must always use an overflow of -1, except for x = 0)
        ForeignField.negate(x, y - 1n);
        return;
    }
    // case of two variables or x = constant and y = variable
    // we compute z = y - x - 1 and check that z \in [0, 2^3l), which implies x < y as above
    // we use modulo 0 here, which means we're proving:
    // z = y - x - 1 - 0*(o1 + o2) for some overflows o1, o2
    sum([y_, x, Field3.from(1n)], [-1n, -1n], 0n);
}
function assertLessThanOrEqual(x, y) {
    (0, common_js_1.assert)(typeof y !== 'bigint' || y >= 0n, 'assertLessThanOrEqual: upper bound must be positive');
    let y_ = Field3.from(y);
    // constant case
    if (Field3.isConstant(x) && Field3.isConstant(y_)) {
        (0, common_js_1.assert)(Field3.toBigint(x) <= Field3.toBigint(y_), 'assertLessThan: got x > y');
        return;
    }
    // provable case
    // we compute z = y - x and check that z \in [0, 2^3l), which implies x <= y
    sum([y_, x], [-1n], 0n);
}
// Field3 from/to bits
function field3FromBits(bits) {
    const Field = (0, field_constructor_js_1.getField)();
    let limbSize = Number(range_check_js_1.l);
    let l0 = Field.fromBits(bits.slice(0 * limbSize, 1 * limbSize));
    let l1 = Field.fromBits(bits.slice(1 * limbSize, 2 * limbSize));
    let l2 = Field.fromBits(bits.slice(2 * limbSize, 3 * limbSize));
    return [l0, l1, l2];
}
exports.field3FromBits = field3FromBits;
// helpers
/**
 * Version of `multiRangeCheck` which does the check on a truncated version of the input,
 * so that it always succeeds, and then checks equality of the truncated and full input.
 *
 * This is a hack to get an error when the constraint fails, around the fact that multiRangeCheck
 * is not checked by snarky.
 */
function indirectMultiRangeChange(x, message = 'multi-range check failed') {
    let xTrunc = (0, exists_js_1.exists)(3, () => {
        let [x0, x1, x2] = toBigint3(x);
        return [x0 & range_check_js_1.lMask, x1 & range_check_js_1.lMask, x2 & range_check_js_1.lMask];
    });
    (0, range_check_js_1.multiRangeCheck)(xTrunc);
    x[0].assertEquals(xTrunc[0], message);
    x[1].assertEquals(xTrunc[1], message);
    x[2].assertEquals(xTrunc[2], message);
}
