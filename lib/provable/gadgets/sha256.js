...
function padding(data) {
    ...
    let k = Number((0, finite_field_js_1.mod)(448n - (BigInt(l) + 1n), 512n));
    ...
    for (let i = 0; i < paddedMessage.length; i += 4) {
        ...
        chunks.push(int_js_1.UInt32.fromBytesBE(paddedMessage.slice(i, i + 4)));
    }
...
const SHA256 = {
    hash(data) {
        ...
        return wrapped_classes_js_1.Bytes.from(H.map((x) => x.toBytesBE()).flat());
    },
    compression: sha256Compression,
    createMessageSchedule,
    padding,
    get initialState() {
        return SHA256Constants.H.map((x) => int_js_1.UInt32.from(x));
    },
};
...
function sha256Compression([...H], W) {
    ...
    for (let t = 0; t <= 63; t++) {
        ...
        g = f;
        f = e;
        e = int_js_1.UInt32.Unsafe.fromField((0, arithmetic_js_1.divMod32)(d.value.add(unreducedT1), 48).remainder); // mod 32bit the unreduced field element
        d = c;
        c = b;
        b = a;
        a = int_js_1.UInt32.Unsafe.fromField((0, arithmetic_js_1.divMod32)(unreducedT2.add(unreducedT1), 48).remainder); // mod 32bit
    }
...
function createMessageSchedule(M) {
    ...
    for (let t = 16; t <= 63; t++) {
        ...
        W[t] = int_js_1.UInt32.Unsafe.fromField((0, arithmetic_js_1.divMod32)(unreduced, 48).remainder);
    }
    return W;
}
