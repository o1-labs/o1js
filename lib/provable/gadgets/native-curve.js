...
exports.add = exports.shiftedScalarToField3 = exports.field3ToShiftedScalar = exports.fieldToShiftedScalar = exports.scaleShifted = exports.scaleField = void 0;
...
function scaleField(P, s) {
    ...
    if (x.isConstant() && y.isConstant() && s.isConstant()) {
        let sP = elliptic_curve_js_1.PallasAffine.scale(elliptic_curve_js_1.PallasAffine.fromNonzero({ x: x.toBigInt(), y: y.toBigInt() }), s.toBigInt());
        return { x: (0, field_constructor_js_1.createField)(sP.x), y: (0, field_constructor_js_1.createField)(sP.y) };
    }
    const Field = (0, field_constructor_js_1.getField)();
    const Point = (0, provable_derivers_js_1.provable)({ x: Field, y: Field });
    /**
     * Strategy:
    ...
    let { isOdd: sLo, high: sHi } = (0, comparison_js_1.isOddAndHigh)(s);
    let shift = finite_field_js_1.Fq.mod(-(1n << 255n));
    (0, common_js_1.assert)((shift & 1n) === 0n); // shift happens to be even, so we don't need to worry about a carry
    ...
    (0, common_js_1.assert)(shiftHi < finite_field_js_1.Fp.modulus / 2n);
    ...
    let tHiSafe = provable_js_1.Provable.if(isEdgeCase, (0, field_constructor_js_1.createField)(0n), tHi);
    // R = (2*(t >> 1) + 1 + 2^255)P
    // also returns a 255-bit representation of tHi
    let [, RMl, [, ...tHiBitsMl]] = bindings_js_1.Snarky.group.scaleFastUnpack([0, x.value, y.value], [0, tHiSafe.value], 255);
    let R = { x: (0, field_constructor_js_1.createField)(RMl[1]), y: (0, field_constructor_js_1.createField)(RMl[2]) };
    // prove that tHi has only 254 bits set
    (0, field_constructor_js_1.createField)(tHiBitsMl[254]).assertEquals(0n);
    // R = tLo ? R : R - P = (t + 2^255)P = sP
    let RminusP = addNonZero(R, negate(P)); // can only be zero if s = 0, which we handle later
    R = provable_js_1.Provable.if(tLo, Point, R, RminusP);
    // now handle the two edge cases s=0 and s=1
    let zero = (0, field_constructor_js_1.createField)(0n);
    let zeroPoint = { x: zero, y: zero };
    let edgeCaseResult = provable_js_1.Provable.if(sLo, Point, P, zeroPoint);
    return provable_js_1.Provable.if(isEdgeCase, Point, edgeCaseResult, R);
}
exports.scaleField = scaleField;
/**
...
function scaleShifted({ x, y }, { lowBit: tLo, high254: tHi }, numHighBits = 254) {
    // constant case
    if ((0, common_js_1.isConstant)(x, y, tHi, tLo)) {
        let sP = elliptic_curve_js_1.PallasAffine.scale(elliptic_curve_js_1.PallasAffine.fromNonzero({ x: x.toBigInt(), y: y.toBigInt() }), finite_field_js_1.Fq.mod(tLo.toField().toBigInt() + 2n * tHi.toBigInt() + (1n << 255n)));
        return { x: (0, field_constructor_js_1.createField)(sP.x), y: (0, field_constructor_js_1.createField)(sP.y) };
    }
    const Field = (0, field_constructor_js_1.getField)();
    const Point = (0, provable_derivers_js_1.provable)({ x: Field, y: Field });
     ...
     */
    let equalsMinusShift = tHi.equals(finite_field_js_1.Fq.modulus - (1n << 254n));
    let equalsMinusShiftMinus1 = tHi.equals(finite_field_js_1.Fq.modulus - (1n << 254n) - 1n);
    let isEdgeCase = equalsMinusShift.or(equalsMinusShiftMinus1);
    let tHiSafe = provable_js_1.Provable.if(isEdgeCase, zero, tHi);
    // R = (2*(t >> 1) + 1 + 2^255)P
    // also returns a 255-bit representation of tHi
    let [, RMl, [, ...tHiBitsMl]] = bindings_js_1.Snarky.group.scaleFastUnpack([0, x.value, y.value], [0, tHiSafe.value], 255);
    let P = { x, y };
    ...
    for (let i = numHighBits; i < 255; i++) {
        (0, field_constructor_js_1.createField)(tHiBitsMl[i]).assertEquals(zero);
    }
    // handle edge cases
    // 2*(-2^254) + 1 + 2^255 = 1
    // 2*(-2^254 - 1) + 1 + 2^255 = -1
    ...
    R = provable_js_1.Provable.if(isEdgeCase, Point, edgeCaseResult, R);
    ...
    RminusP = provable_js_1.Provable.if(isInfinity, Point, { x: zero, y: zero }, RminusP);
    R = provable_js_1.Provable.if(tLo, Point, R, RminusP);
    return R;
}
exports.scaleShifted = scaleShifted;
/**
...
function fieldToShiftedScalar(s) {
    // constant case
    if (s.isConstant()) {
        let t = finite_field_js_1.Fq.mod(s.toBigInt() - (1n << 255n));
        let lowBit = (0, field_constructor_js_1.createBool)((t & 1n) === 1n);
        let high254 = (0, field_constructor_js_1.createField)(t >> 1n);
...
exports.fieldToShiftedScalar = fieldToShiftedScalar;
/**
...
function field3ToShiftedScalar(s) {
    // constant case
    if (foreign_field_js_1.Field3.isConstant(s)) {
        let t = finite_field_js_1.Fq.mod(foreign_field_js_1.Field3.toBigint(s) - (1n << 255n));
        let lowBit = (0, field_constructor_js_1.createBool)((t & 1n) === 1n);
        let high254 = (0, field_constructor_js_1.createField)(t >> 1n);
        return { lowBit, high254 };
    }
    // compute t = s - (2^255 mod q) using foreign field subtraction
    let twoTo255 = foreign_field_js_1.Field3.from(finite_field_js_1.Fq.mod(1n << 255n));
    let t = foreign_field_js_1.ForeignField.sub(s, twoTo255, finite_field_js_1.Fq.modulus);
    let [t0, t1, t2] = t;
    ...
    foreign_field_js_1.ForeignField.assertLessThan(t, finite_field_js_1.Fq.modulus);
    // split t into 254 high bits and a low bit
    // => split t0 into [1, 87] => split t0 into [1, 64, 23] so we can efficiently range-check
    let [tLo, tHi00, tHi01] = (0, exists_js_1.exists)(3, () => {
        let t = t0.toBigInt();
        return [(0, common_js_1.bit)(t, 0), (0, common_js_1.bitSlice)(t, 1, 64), (0, common_js_1.bitSlice)(t, 65, 23)];
    });
    let tLoBool = tLo.assertBool();
    (0, range_check_js_1.rangeCheck64)(tHi00);
    (0, range_check_js_1.rangeCheck64)(tHi01);
    ...
    let tHi = tHi0
        .add(t1.mul(1n << (range_check_js_1.l - 1n)))
        .add(t2.mul(1n << (2n * range_check_js_1.l - 1n)))
        .seal();
    return { lowBit: tLoBool, high254: tHi };
}
exports.field3ToShiftedScalar = field3ToShiftedScalar;
/**
...
function shiftedScalarToField3(t) {
    ...
    if (t.high254.isConstant() && t.lowBit.isConstant()) {
        ...
        return foreign_field_js_1.Field3.from(finite_field_js_1.Fq.mod(s + (1n << 255n)));
    }
    // split sHi into 64, 23, 88, 79 bit limbs (254 in total)
    let [t0Hi0, t0Hi1, t1, t2] = (0, exists_js_1.exists)(4, () => {
        let tHi = t.high254.toBigInt();
        return [
            (0, common_js_1.bitSlice)(tHi, 0, 64),
            (0, common_js_1.bitSlice)(tHi, 64, 23),
            (0, common_js_1.bitSlice)(tHi, 87, 88),
            (0, common_js_1.bitSlice)(tHi, 175, 79),
        ];
    });
    // check t0 < 2^88 in pieces and reassemble
    (0, range_check_js_1.rangeCheck64)(t0Hi0);
    (0, range_check_js_1.rangeCheckLessThan64)(23, t0Hi1);
    ...
    (0, range_check_js_1.multiRangeCheck)([t1, t2, t2.mul(1n << 9n)]); // 2^9 t2 < 2^88 => t2 < 2^79
    ...
    return foreign_field_js_1.ForeignField.sum([tBig, foreign_field_js_1.Field3.from(shift), foreign_field_js_1.Field3.from(0n)], [1n, 1n], finite_field_js_1.Fq.modulus);
}
exports.shiftedScalarToField3 = shiftedScalarToField3;
/**
 * Wraps the `EC_add` gate to perform complete addition of two non-zero curve points.
 */
function add(g, h) {
    // compute witnesses
    let witnesses = (0, exists_js_1.exists)(7, () => {
        let x1 = g.x.toBigInt();
        let y1 = g.y.toBigInt();
        let x2 = h.x.toBigInt();
        let y2 = h.y.toBigInt();
        ...
        let infZ = sameX ? finite_field_js_1.Fp.inverse(y2 - y1) ?? 0n : 0n;
        let x21Inv = finite_field_js_1.Fp.inverse(x2 - x1) ?? 0n;
        let slopeDouble = finite_field_js_1.Fp.div(3n * x1 ** 2n, 2n * y1) ?? 0n;
        let slopeAdd = finite_field_js_1.Fp.mul(y2 - y1, x21Inv);
        let s = sameX ? slopeDouble : slopeAdd;
        let x3 = finite_field_js_1.Fp.mod(s ** 2n - x1 - x2);
        let y3 = finite_field_js_1.Fp.mod(s * (x1 - x3) - y1);
        return [sameX, inf, infZ, x21Inv, s, x3, y3];
    });
    let [same_x, inf, inf_z, x21_inv, s, x3, y3] = witnesses;
    bindings_js_1.Snarky.gates.ecAdd((0, base_js_1.MlPair)(g.x.seal().value, g.y.seal().value), (0, base_js_1.MlPair)(h.x.seal().value, h.y.seal().value), (0, base_js_1.MlPair)(x3.value, y3.value), inf.value, same_x.value, s.value, inf_z.value, x21_inv.value);
    // the ecAdd gate constrains `inf` to be boolean
    let isInfinity = (0, field_constructor_js_1.createBoolUnsafe)(inf);
