...
function createProvableBigInt(modulus, config) {
    ...
    class ProvableBigInt_ extends ProvableBigInt {
         ...
         */
        static max() {
            return _a.fromBigInt(modulus - 1n);
        }
        /**
        ...
        static fromBigInt(x) {
            ...
            if (value >= _a.modulus.toBigInt()) {
                value = value % modulus;
            }
            let fields = bigintToLimbs(value, _a.config);
            return new _a(fields, unconstrained_js_1.Unconstrained.from(value));
        }
        /**
         ...
         */
        clone() {
            return new _a(this.fields, this.value);
        }
        /**
        ...
        add(a, isDouble = false) {
            ...
            let { q, r } = provable_js_1.Provable.witness((0, provable_derivers_js_1.provable)({ q: _a, r: _a }), () => {
         ...
         */
        inverse() {
            let { res } = provable_js_1.Provable.witness((0, provable_derivers_js_1.provable)({ res: _a }), () => {
         ...
         */
        negate() {
            let { negation } = provable_js_1.Provable.witness((0, provable_derivers_js_1.provable)({ negation: _a }), () => {
        ...
        pow(exp) {
            ...
            for (const chunk of processChunk(exponentBits, 100)) {
                for (const bit of chunk) {
                    result = provable_js_1.Provable.if(bit, _a, result.mul(base), result);
                    base = base.mul(base);
                }
            }
            return result;
         ...
         */
        sqrt() {
            let r = provable_js_1.Provable.witness(_a, () => {
                const p = this.Constructor.modulus.toBigInt();
                // Special cases
    ...
    _a = ProvableBigInt_;
    (() => {
        _a._modulus = new _a(fields, unconstrained_js_1.Unconstrained.from(modulus));
        _a._config = config_;
    ...
    ProvableBigInt_.Unsafe = {
        ...
        fromBits(bits) {
            let value = 0n;
            let bigint = provable_js_1.Provable.witness(_a, () => {
                for (let i = 0; i < bits.length; i++) {
                    if (bits[i].toBoolean()) {
                        value |= 1n << BigInt(i);
                    }
                }
                return _a.fromBigInt(value);
            });
            return bigint;
        },
    };
