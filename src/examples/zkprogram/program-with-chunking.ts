...
async function proveWithNative() {
  ...
  let roundtripProof = await MyProgram.Proof.fromJSON(proof.toJSON());
  let roundtripProofIsValid = await MyProgram.verify(roundtripProof);

  await persistArtifacts(proof.toJSON(), verificationKey, cs.baseCase.rows);
  ...
  console.log('');
  console.log(
    'Native verify control: ./run src/examples/zkprogram/program-with-chunking.ts verify-native'
  );
  console.log(
    'Next run: ./run src/examples/zkprogram/program-with-chunking.ts verify-wasm'
  );
}
...
async function verifyWithBackend(backend: 'native' | 'wasm') {
  ...
  let MyProgram = createChunkingProgram();
  let cache = Cache.FileSystem(cacheDir, true);
  let perf = Performance.create('example-with-chunking');
...
async function loadArtifacts(): Promise<{
  ...
  return {
    ...
    metadata:
      metadataJson === null ? null : (JSON.parse(metadataJson) as { rows?: number }),
  };
}
...
function getArtifactsDir() {
  ...
  return path.resolve('tests/test-artifacts/program-with-chunking-cross-backend');
}

function printUsageAndExit(mode: string): never {
  throw new Error(
    `Unknown mode "${mode}". Use one of:\n` +
    `  ./run src/examples/zkprogram/program-with-chunking.ts prove-native\n` +
    `  ./run src/examples/zkprogram/program-with-chunking.ts verify-native\n` +
    `  ./run src/examples/zkprogram/program-with-chunking.ts verify-wasm\n` +
    `Optional:\n` +
