...
import { UInt32, Provable } from 'o1js';
import { bitsToBytes } from 'src/bindings/lib/binable.js';
...
function processStringToMessageBlocks(s: string) {
  ...
  msgBits = msgBits + '1';
  console.log('msgBits:', msgBits.length);
...
const SHA256 = {
  hash(msg: string) {
    ...
    let messageBlocks = Provable.witness(
      Provable.Array(Provable.Array(UInt32, 16), 6),
      () => {
        console.log('hashing <' + msg + '>');
        return processStringToMessageBlocks(msg);
      }
    );
...
let cs = Provable.constraintSystem(() => SHA256.hash('test'));
