...
export class Main extends C {
  ...
  static main(witness: Witness, @public_ percentChange: Field) {
    ...
    sellTotal
      .mul(FULL_BASIS.add(percentChange))
      .assertGte(buyTotal.mul(FULL_BASIS));
  }
}
...
export function main() {
  ...
  const proof = Main.prove(
    [
      {
        buyIndex: new Field(0),
        sellIndex: new Field(1),
        attestation: new HTTPSAttestation(
          new Bytes([
            trade({
              timestamp: new Field(150),
              price: new Field(100),
              quantity: new Field(5),
              isBuy: new Bool(true),
            }),
            trade({
              timestamp: new Field(250),
              price: new Field(500),
              quantity: new Field(4),
              isBuy: new Bool(false),
            }),
          ]),
