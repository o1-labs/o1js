 ...
 */
export class SparkyDebuggingExtensions implements BaseExtension {
  public readonly name = 'SparkyDebugging';
  public readonly version = '1.0.0';
  
  private _isActive = false;
  private _config: DebugConfig;
  private _debugEntries: DebugEntry[] = [];
  private _constraintTraces: ConstraintTrace[] = [];
  private _variableTraces: Map<number, string[]> = new Map();
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new DebugError(
        `Failed to initialize debugging extensions: ${errorMessage}`,
        'INIT_FAILED'
      );
    }
  }
   ...
   */
  public getDebugLevel(): DebugLevel {
   ...
   */
  public clearDebugEntries(level?: DebugLevel, category?: string): void {
    ensureSparkyBackend();
  ...
  public traceConstraint(
    ...
    variables: number[],
    coefficients: string[],
    sourceLocation?: { function: string; line?: number; column?: number }
  ): void {
    ...
    this.log(
      DebugLevel.TRACE,
      'constraints',
      `Constraint generated: ${operation}`,
      {
        variables,
        coefficients,
        sourceLocation
   ...
   */
  public async getConstraintDebugInfo(): Promise<ConstraintDebugInfo[]> {
    ensureSparkyBackend();
   ...
   */
  public traceVariable(variableId: number, context: string): void {
    ...
    this.log(
      DebugLevel.TRACE,
      'variables',
      `Variable ${variableId} allocated in: ${context}`
    );
  }
   ...
   */
  public getVariableTrace(variableId: number): string[] {
   ...
   */
  private createDefaultConfig(): DebugConfig {
    return {
   ...
   */
  private shouldLogToConsole(level: DebugLevel): boolean {
