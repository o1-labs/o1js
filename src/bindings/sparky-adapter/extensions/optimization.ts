 ...
 */
const OPTIMIZATION_PRESETS: Record<OptimizationLevel, OptimizationConfig> = {
 ...
 */
export class SparkyOptimizationExtensions implements BaseExtension {
  public readonly name = 'SparkyOptimization';
  public readonly version = '1.0.0';
  
  private _isActive = false;
  private _currentLevel: OptimizationLevel = OptimizationLevel.AGGRESSIVE;
  private _customConfig: OptimizationConfig = { ...OPTIMIZATION_PRESETS.custom };
  private _stats: OptimizationStats;
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new OptimizationError(
        `Failed to initialize optimization extensions: ${errorMessage}`,
        'INIT_FAILED'
      );
    }
  }
   ...
   */
  public async setOptimizationLevel(level: OptimizationLevel): Promise<void> {
    ensureSparkyBackend();
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new OptimizationError(
        `Failed to set optimization level to ${level}: ${errorMessage}`,
        'LEVEL_SET_FAILED'
      );
    }
  }
   ...
   */
  public async getOptimizationLevel(): Promise<OptimizationLevel> {
    ensureSparkyBackend();
   ...
   */
  public async setCustomConfig(config: Partial<OptimizationConfig>): Promise<void> {
    ensureSparkyBackend();
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new OptimizationError(
        `Failed to set custom optimization config: ${errorMessage}`,
        'CUSTOM_CONFIG_FAILED'
      );
    }
  }
   ...
   * @extension This method is Sparky-specific and NOT part of original Snarky API
   */
  public async resetOptimizationStats(): Promise<void> {
    ensureSparkyBackend();
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new OptimizationError(
        `Failed to reset optimization stats: ${errorMessage}`,
        'STATS_RESET_FAILED'
      );
    }
  }
   ...
   */
  private levelToWasmMode(level: OptimizationLevel): string {
    switch (level) {
   ...
   */
  private wasmModeToLevel(mode: string): OptimizationLevel {
    switch (mode) {
      case 'none':
   ...
   */
  private updateStatsConfig(config: OptimizationConfig): void {
   ...
   */
  private updateStatsFromWasm(wasmStats: any): void {
    if (!wasmStats) return;
    ...
    if (this._stats.constraintsBefore > 0) {
      this._stats.effectiveness = Math.round(
        (this._stats.constraintsEliminated / this._stats.constraintsBefore) * 100
      );
    }
  }
   ...
   */
  private validateOptimizationConfig(config: Partial<OptimizationConfig>): void {
    const validKeys = [
    ...
    for (const key of Object.keys(config)) {
      if (!validKeys.includes(key)) {
        throw new OptimizationError(
          `Invalid optimization config key: ${key}`,
          'INVALID_CONFIG_KEY'
        );
      }
      
      if (typeof config[key as keyof OptimizationConfig] !== 'boolean') {
        throw new OptimizationError(
          `Optimization config value for ${key} must be boolean`,
