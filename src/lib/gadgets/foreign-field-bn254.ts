...
const ForeignFieldBn254 = {
  ...
  assertLessThan(x: Field3Bn254, f: bigint) {
    ...
    ForeignFieldBn254.negate(x, f - 1n);
  },
...
function inverse(x: Field3Bn254, f: bigint): Field3Bn254 {
  ...
  let one: Field2 = [FieldBn254.from(1n), FieldBn254.from(0n)];
  assertMulInternal(x, xInv, one, f);

  // range check on result bound
  // TODO: this uses two RCs too many.. need global RC stack
  multiRangeCheck([xInv2Bound, FieldBn254.from(0n), FieldBn254.from(0n)]);
...
function divide(
  ...
  // range check on result bound
  multiRangeCheck([z2Bound, FieldBn254.from(0n), FieldBn254.from(0n)]);

  if (!allowZeroOverZero) {
    ForeignFieldBn254.equals(y, 0n, f).assertFalse();
  }
  return z;
}
...
function weakBound(x: FieldBn254, f: bigint) {
...
function assertAlmostReduced(xs: Field3Bn254[], f: bigint, skipMrc = false) {
  ...
  if (TupleN.hasLength(1, bounds)) {
    multiRangeCheck([...bounds, FieldBn254.from(0n), FieldBn254.from(0n)]);
  }
  if (TupleN.hasLength(2, bounds)) {
    multiRangeCheck([...bounds, FieldBn254.from(0n)]);
  }
}
...
type Field2 = [FieldBn254, FieldBn254];
const Field2 = {
...
class Sum {
  ...
  finish(f: bigint, isChained = false) {
    ...
    if (!isChained) GatesBn254.zero(...result);
  ...
  finishForMulInput(f: bigint, isChained = false) {
    ...
    if (!isChained) GatesBn254.zero(...x);
