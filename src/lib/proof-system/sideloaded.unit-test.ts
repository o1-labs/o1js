...
const sideloadedProgram = ZkProgram({
    ...
    methods: {
        ...
        recurseTwoSideloaded: {
            privateInputs: [SampleSideloadedProof, VerificationKey, SampleSideloadedProof2, VerificationKey],
            async method(publicInput: Field, proof1: SampleSideloadedProof, vk1: VerificationKey, proof2: SampleSideloadedProof2, vk2: VerificationKey) {
                proof1.verify(vk1);
                proof2.verify(vk2);

                proof1.publicInput.add(proof2.publicInput.field1.add(proof2.publicInput.field2)).assertEquals(publicInput, "PublicInput not matching");
            }
        }
    }
})
    ...
    methods: {
        ...
        recurseTwoSideloaded: {
            ...
            async method(publicInput: Field, proof1: SampleSideloadedProof, vk1: VerificationKey, proof2: SampleSideloadedProof2, vk2: VerificationKey) {
                ...
                proof1.publicInput.add(proof2.publicInput.field1.add(proof2.publicInput.field2)).add(1).assertEquals(publicInput, "PublicInput not matching");
            }
        }
    }
})
...
describe("sideloaded", () => {
    ...
    it("recurse one proof with zkprogram", async () => {
        ...
        const finalProof = await sideloadedProgram.recurseOneSideloaded(Field(1), proof, program1Vk);
