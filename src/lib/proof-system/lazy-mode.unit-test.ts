...
async function testLazyMode(lazyMode: boolean) {
    ...
    console.log(' ');
    console.log(`(${lazyMode ? 'Lazy' : 'Eager'}) Memory before proof (1)`, getMemory());
    
    let { proof } = await LazyMode.baseCase(new Field(1n));
    
    console.log(`(${lazyMode ? 'Lazy' : 'Eager'}) Memory after proof (1)`, getMemory()); 
    console.log(' ');

    let isValid = await LazyMode.verify(proof);
  
    console.log(`(${lazyMode ? 'Lazy' : 'Eager'}) Memory after verify (1)`, getMemory());
