...
function runAndCheckBn254(f: () => void) {
  ...
  try {
    Snarky.bn254.run.runAndCheck(f);
  } catch (error) {
...
function runUncheckedBn254(f: () => void) {
  let id = snarkContext.enter({ inCheckedComputation: true });
  try {
    Snarky.bn254.run.runUnchecked(f);
  } catch (error) {
    throw prettifyStacktrace(error);
  } finally {
    snarkContext.leave(id);
...
function constraintSystemBn254<T>(f: () => T) {
  let id = snarkContext.enter({ inAnalyze: true, inCheckedComputation: true });
  try {
    let result: T;
    let { rows, digest, json } = Snarky.bn254.run.constraintSystem(() => {
      result = f();
    });
