...
function createProvableBigInt(modulus: bigint, config?: BigIntParameter) {
    ...
    assert(modulus < config_.MAX, `ProvableBigInt: modulus exceeds the max supported size of 2^${config_.MAX}`);
    let fields = [];
    let x = modulus;
    if (x < 0n) {
    ...
    if (x > config_.MAX) {
        throw new Error(`Modulus exceeds ${config_.limb_num * Number(config_.limb_size)}-bit size limit.`);
    }
        ...
        * @returns A ProvableBigInt representing zero
        ...
        sub(a: ProvableBigInt_): ProvableBigInt_ {
            ...
            for (let i = 0; i < this.Constructor.config.limb_num; i++) {
                ...
                borrow = diff.lessThan(Field.from(this.Constructor.config.mask + 1n)).not().toField();
                fields.push(diff.add(borrow.mul(Field.from(this.Constructor.config.mask + 1n))));
                rangeCheck(fields[i], Number(this.Constructor.config.limb_size));
            }
        ...
        div(
            a: ProvableBigInt_,
        ): { quotient: ProvableBigInt_; remainder: ProvableBigInt_ } {
            let { q, r } = Provable.witness(
                Struct({ q: ProvableBigInt_ as typeof ProvableBigInt, r: ProvableBigInt_ as typeof ProvableBigInt }),
                () => {
...
function findConfig(modulus: bigint): BigIntParameter {
    ...
    if (filteredParams.length === 0) {
        throw new Error(`No configuration found for modulus ${modulus}. It exceeds the maximum supported size.`);
    }
