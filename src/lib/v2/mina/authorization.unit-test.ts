import { AuthorizationLevel, AuthorizationLevelIdentifier } from './authorization.js';
...
{
  const identityPredicates: {[key in AuthorizationLevelIdentifier]: (x: AuthorizationLevel) => Bool} = {
  ...
  function testExclusiveIdentity(authLevel: AuthorizationLevel, expectedIdentity: AuthorizationLevelIdentifier) {
    Object.entries(identityPredicates).forEach(([identifier, predicate]) => {
      expect(predicate(authLevel).toBoolean()).toBe(identifier === expectedIdentity);
    });
  }
...
>(v1FetchLayout(layoutV1.AccountUpdate, ['body', 'update', 'permissions', 'editState']));

{
  testV1V2ClassEquivalence(V1AuthorizationLevel, AuthorizationLevel);
