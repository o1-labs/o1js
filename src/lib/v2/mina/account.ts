...
import { StateDefinition, StateLayout, StateUpdates, StateValues } from './state.js';
...
function accountIdKeys(accountId: AccountId): {publicKey: string, tokenId: string} {
  return {
...
export class AccountId {
  ...
  equals(x: AccountId): Bool {
    return Bool.allTrue([
      this.publicKey.equals(x.publicKey),
      this.tokenId.equals(x.tokenId)
    ]);
  }
...
export class Account<State extends StateLayout = 'GenericState'> {
  ...
  // TODO: error handling
  checkAndApplyFeePayment(feePayment: ZkappFeePayment):
    ...
    | {status: 'Failed', errors: Error[]}
  {
    ...
    if(this.accountId.publicKey.equals(feePayment.publicKey).not().toBoolean())
      errors.push(new Error('fee payment public key does not match account public key'));
    ...
    if(this.balance.lessThan(feePayment.fee).toBoolean())
      errors.push(new Error('account does not have enough balance to pay the required fee'));
  ...
  checkAndApplyUpdate<Event, Action>(update: AccountUpdate<State, Event, Action>):
      {status: 'Applied', updatedAccount: Account<State>}
    | {status: 'Failed', errors: Error[]}
  {
    const errors: Error[] = [];
    ...
    checkPrecondition<UInt64>('balance', update.preconditions.account.balance, this.balance);
    checkPrecondition<UInt32>('nonce', update.preconditions.account.nonce, this.nonce);
    checkPrecondition<Field>('receiptChainHash', update.preconditions.account.receiptChainHash, this.receiptChainHash);
    if(this.delegate !== null) checkPrecondition<PublicKey>('delegate', update.preconditions.account.delegate, this.delegate);
    checkPrecondition<Bool>('isProven', update.preconditions.account.isProven, this.zkapp.isProven);

    // checkPrecondition('state', update.preconditions.account.state, this.state);

    StateValues.checkPreconditions(this.State, this.zkapp.state, update.preconditions.account.state);

    const actionState = this.zkapp?.actionState ?? [];
    const actionStateSatisfied = Bool.anyTrue(actionState.map((s) => update.preconditions.account.actionState.isSatisfied(s)));
    if(actionStateSatisfied.not().toBoolean())
      errors.push(preconditionError('actionState', update.preconditions.account.actionState, actionState));
    ...
    if(
      ...
      errors.push(new Error(`insufficient balance for balanceChange (balance = ${this.balance}, balanceChange = -${update.balanceChange.magnitude})`));
    } else {
      // TODO: check for overflows?
      const isPos = update.balanceChange.isPositive().toBoolean();
      const amount = update.balanceChange.magnitude;
      updatedBalance = isPos ? this.balance.add(amount) : this.balance.sub(amount);
    }
    ...
    if(errors.length === 0) {
      ...
      const allStateUpdated = Bool.allTrue(StateUpdates.toFieldUpdates(this.State, update.stateUpdates).map((update) => update.set));
      ...
      const updatedAccount = new Account(
        this.State,
        {
          ...
          nonce: update.incrementNonce.toBoolean() ? this.nonce.add(UInt32.one) : this.nonce,
          ...
          zkapp: {
            state: StateValues.applyUpdates(this.State, this.zkapp.state, update.stateUpdates),
            verificationKey: applyUpdate(update.verificationKeyUpdate, this.zkapp.verificationKey),
            // actionState: TODO,
