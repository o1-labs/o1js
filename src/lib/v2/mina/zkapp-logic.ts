...
function tryAddInt64(a: Int64, b: Int64): Int64 | null {
  if (
    a.sgn.equals(b.sgn).toBoolean() &&
    a.magnitude.lessThan(b.magnitude).toBoolean()
  )
    return null;
...
function checkPreconditions<State extends StateLayout>(
  ...
  if (actionStateSatisfied.not().toBoolean())
    errors.push(
      preconditionError(
        'actionState',
        preconditions.account.actionState,
        actionState
      )
    );
...
function checkPermissions<State extends StateLayout, Event, Action>(
  ...
  function checkPermission(
    ...
    if (
      actionIsPerformed &&
      !requiredAuthLevel.isSatisfied(update.authorizationKind)
    )
      errors.push(
        new Error(
  ...
  checkPermission(
    'receive',
    permissions.receive,
    update.balanceChange.isPositive().toBoolean()
  );
  checkPermission(
    'incrementNonce',
    permissions.incrementNonce,
    update.incrementNonce.toBoolean()
  );
  checkPermission(
    'setDelegate',
    permissions.setDelegate,
    update.delegateUpdate.set.toBoolean()
  );
  checkPermission(
    'setPermissions',
  ...
  checkPermission(
    ...
    update.zkappUriUpdate.set.toBoolean()
  );
  checkPermission(
    'setTokenSymbol',
  ...
  checkPermission(
    'setVotingFor',
    permissions.setVotingFor,
    update.votingForUpdate.set.toBoolean()
  );
  checkPermission(
    'setTiming',
    permissions.setTiming,
    update.timingUpdate.set.toBoolean()
  );
  checkPermission(
    'editActionState',
...
function applyUpdates<State extends StateLayout, Event, Action>(
  ...
  if (account.isNew.get()) {
    ...
    feeExcessState = updateApplyState(
      ...
      (feeExcess) =>
        ...
        new Error(
          'fee excess underflowed due when subtracting the account creation fee'
        )
    );

    if (update.implicitAccountCreationFee.toBoolean()) {
      const balanceChangeWithoutCreationFee = tryAddInt64(
        actualBalanceChange,
        accountCreationFee
      );
      if (balanceChangeWithoutCreationFee === null) {
        errors.push(
          new Error(
            'balance change underflowed when subtracting the account creation fee'
          )
        );
      } else {
  ...
  if (updatedBalanceSigned === null) {
    errors.push(
      new Error(
        'account balance overflowed or underflowed when applying balance change'
      )
    );
  } else if (updatedBalanceSigned.isNegative().toBoolean()) {
    errors.push(
      new Error('account balance was negative after applying balance change')
    );
  } else {
  ...
  const allStateUpdated = Bool.allTrue(
    StateUpdates.toFieldUpdates(account.State, update.stateUpdates).map(
      (update) => update.set
    )
  );
...
//       account update.
export function checkAndApplyAccountUpdate<
...
>(
  ...
  if (!account.accountId.equals(update.accountId).toBoolean())
    errors.push(
      new Error('account id in account update does not match actual account id')
    );

  if (
    !account.zkapp.verificationKey.hash
      .equals(update.verificationKeyHash)
      .toBoolean()
  )
    errors.push(
      new Error(
