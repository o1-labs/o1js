...
export namespace Precondition {
  ...
  export class InRange<T extends Compare<T>> {
    ...
    static disabled<T extends Compare<T>>(
    ...
    static betweenInclusive<T extends Compare<T>>(
      lower: T,
      upper: T
    ): InRange<T> {
    ...
    static fromOption<T extends Compare<T>>(
      option: Option<Range<T>>
    ): InRange<T> {
...
export type PreconditionsDescription<State extends StateLayout> = {
  ...
  account?:
    | AccountPreconditionsDescription<State>
...
export class Preconditions<State extends StateLayout = 'GenericState'> {
  ...
  constructor(
    ...
    descr?: PreconditionsDescription<State>
  ) {
  ...
  static toInput<State extends StateLayout>(
    x: Preconditions<State>
  ): HashInput {
  ...
  static fromFields(fields: Field[], aux: any[]): Preconditions {
    return Preconditions.fromInternalRepr(
      BindingsLayout.Preconditions.fromFields(fields, aux)
    );
  }
...
export class EpochLedgerPreconditions {
  ...
  static fromInternalRepr(
    x: BindingsLayout.EpochLedgerPrecondition
  ...
  static toAuxiliary(x?: EpochLedgerPreconditions): any[] {
    return BindingsLayout.EpochLedgerPrecondition.toAuxiliary(
      x?.toInternalRepr()
    );
  }
  ...
  static from(
    value:
      | EpochLedgerPreconditions
      | EpochLedgerPreconditionsDescription
      | undefined
  ): EpochLedgerPreconditions {
...
export class EpochDataPreconditions {
  ...
  constructor(descr?: EpochDataPreconditionsDescription) {
    ...
    this.startCheckpoint = Precondition.Equals.from(
      descr?.startCheckpoint,
      new Field(0)
    );
    this.lockCheckpoint = Precondition.Equals.from(
      descr?.lockCheckpoint,
      new Field(0)
    );
  ...
  static fromInternalRepr(
    x: BindingsLayout.EpochDataPrecondition
  ...
  static toAuxiliary(x?: EpochDataPreconditions): any[] {
    return BindingsLayout.EpochDataPrecondition.toAuxiliary(
      x?.toInternalRepr()
    );
  }
  ...
  static from(
    value:
      | EpochDataPreconditions
      | EpochDataPreconditionsDescription
      | undefined
  ): EpochDataPreconditions {
...
export class NetworkPreconditions {
  ...
  constructor(descr?: NetworkPreconditionsDescription) {
    this.snarkedLedgerHash = Precondition.Equals.from(
      descr?.snarkedLedgerHash,
      Field.empty()
    );
    ...
    this.globalSlotSinceGenesis = Precondition.InRange.from(
      descr?.globalSlotSinceGenesis,
      {
        lower: UInt32.empty(),
        upper: UInt32.MAXINT(),
      }
    );
    this.stakingEpochData = EpochDataPreconditions.from(
      descr?.stakingEpochData
    );
  ...
  static fromInternalRepr(
    x: BindingsLayout.NetworkPrecondition
    ...
    return new NetworkPreconditions({
      ...
      totalCurrency: Precondition.InRange.fromOption(x.totalCurrency),
      globalSlotSinceGenesis: Precondition.InRange.fromOption(
        x.globalSlotSinceGenesis
      ),
      stakingEpochData: EpochDataPreconditions.fromInternalRepr(
        x.stakingEpochData
      ),
...
export class AccountPreconditions<State extends StateLayout = 'GenericState'> {
  ...
  constructor(
    State: StateDefinition<State>,
    descr?: AccountPreconditionsDescription<State>
  ) {
    this.State = State;
    ...
    this.receiptChainHash = Precondition.Equals.from(
      descr?.receiptChainHash,
      Field.empty()
    );
    this.delegate = Precondition.Equals.from(
      descr?.delegate,
      PublicKey.empty()
    );
  ...
  toInternalRepr(): BindingsLayout.AccountPrecondition {
    const statePreconditions = StatePreconditions.toFieldPreconditions(
      this.State,
      this.state
    );
  ...
  static fromInternalRepr(
    x: BindingsLayout.AccountPrecondition
    ...
    return new AccountPreconditions<'GenericState'>('GenericState', {
      ...
      state: new GenericStatePreconditions(
        x.state.map(Precondition.Equals.fromOption<Field>)
      ),
  ...
  static generic(
    descr?: AccountPreconditionsDescription<'GenericState'>
  ): AccountPreconditions {
  ...
  static toJSON<State extends StateLayout>(
