...
Constraint System Testing helps you verify that your circuits generate the expected constraints. This is essential for ensuring both the correctness and performance of zkSNARK circuits.

#### `constraintSystem(label, inputs, main, constraintSystemTest)`

Tests properties of the constraint system generated by a circuit. It automatically generates a variety of field variable types (constants, variables, add & scale combinators) to comprehensively test your circuit's behavior.

##### Parameters
- `label`: Description of the constraint system for error reporting
- `inputs`: Input specification in the form `{ from: [...provables] }`
- `main`: The circuit function to test
