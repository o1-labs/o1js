...
function getGatesModule() {
    return (0, module_loader_js_1.getSparkyInstance)().gates;
}
/**
...
function createWitnessVar() {
    ...
    return (0, format_converter_js_1.ensureFieldVar)(cvar);
}
/**
...
exports.gateOperations = {
    ...
    generic(sl, l, sr, r, so, o, sm, sc) {
        ...
        try {
            (0, module_loader_js_1.getSparkyInstance)().gatesRaw(GENERIC_GATE_TYPE, values, coefficients);
        }
        catch (error) {
            throw error;
        }
    ...
    raw(kind, values, coeffs) {
        ...
        (0, module_loader_js_1.getSparkyInstance)().gatesRaw(kind, values, stringCoeffs);
    },
    /**
     * Poseidon hash gate
     */
    ...
    ecScale(state) {
        ...
        try {
            ...
            for (let i = 0; i < accs.length && i < bits.length; i++) {
                ...
                const one = field_operations_js_1.fieldOperations.constant('1');
                const invBit = field_operations_js_1.fieldOperations.sub(one, bit);
                const addX_scaled = field_operations_js_1.fieldOperations.mul(bit, addResult.x);
                const accX_scaled = field_operations_js_1.fieldOperations.mul(invBit, acc[0]);
                const resultX = field_operations_js_1.fieldOperations.add(addX_scaled, accX_scaled);
                const addY_scaled = field_operations_js_1.fieldOperations.mul(bit, addResult.y);
                const accY_scaled = field_operations_js_1.fieldOperations.mul(invBit, acc[1]);
                const resultY = field_operations_js_1.fieldOperations.add(addY_scaled, accY_scaled);
                // Update accumulator for next iteration
                if (i + 1 < accs.length) {
                    getFieldModule().assertEqual((0, format_converter_js_1.fieldVarToCvar)(accs[i + 1][0]), (0, format_converter_js_1.fieldVarToCvar)(resultX));
                    getFieldModule().assertEqual((0, format_converter_js_1.fieldVarToCvar)(accs[i + 1][1]), (0, format_converter_js_1.fieldVarToCvar)(resultY));
                }
            }
            // Process counter progression: nNext = nPrev + 1
            if (nPrev !== undefined && nNext !== undefined) {
                const prevField = field_operations_js_1.fieldOperations.constant(nPrev.toString());
                const nextField = field_operations_js_1.fieldOperations.constant(nNext.toString());
                const oneField = field_operations_js_1.fieldOperations.constant('1');
                const expected = field_operations_js_1.fieldOperations.add(prevField, oneField);
                getFieldModule().assertEqual((0, format_converter_js_1.fieldVarToCvar)(nextField), (0, format_converter_js_1.fieldVarToCvar)(expected));
            }
        }
        catch (error) {
    ...
    ecEndoscale(state) {
        ...
        try {
            ...
            const lambda = field_operations_js_1.fieldOperations.constant('0x2D33357CB532458ED3552A23A8554E5005270D29D19FC7D27B7FD22F0201B547');
            ...
            const endoXp = field_operations_js_1.fieldOperations.mul(lambda, xp);
    ...
    rangeCheck0(x, xLimbs12, xLimbs2, isCompact) {
        ...
        const jsLimbs12 = (0, format_converter_js_1.mlArrayToJsArray)(xLimbs12);
        const jsLimbs2 = (0, format_converter_js_1.mlArrayToJsArray)(xLimbs2);
        ...
        (0, module_loader_js_1.getSparkyInstance)().gates.rangeCheck0(x, jsLimbs12, jsLimbs2, jsIsCompact);
    },
    /**
    ...
    rangeCheck1(v2, v12, vCurr, vNext) {
        // Convert MLArrays to JavaScript arrays for Rust WASM
        const jsVCurr = (0, format_converter_js_1.mlArrayToJsArray)(vCurr);
        const jsVNext = (0, format_converter_js_1.mlArrayToJsArray)(vNext);
        // Cast to any to bypass TypeScript type checking for new methods
        (0, module_loader_js_1.getSparkyInstance)().gates.rangeCheck1(v2, v12, jsVCurr, jsVNext);
    },
    /**
    ...
    foreignFieldAdd(left, right, fieldOverflow, carry, foreignFieldModulus, sign) {
        try {
            ...
            const leftJs = (0, format_converter_js_1.mlArrayToJsArray)(left);
            const rightJs = (0, format_converter_js_1.mlArrayToJsArray)(right);
            const modulusJs = (0, format_converter_js_1.mlArrayToJsArray)(foreignFieldModulus);
