...
exports.SparkyDebuggingExtensions = void 0;
 ...
 */
class SparkyDebuggingExtensions {
    name = 'SparkyDebugging';
    version = '1.0.0';
    _isActive = false;
    _config;
    _debugEntries = [];
    _constraintTraces = [];
    _variableTraces = new Map();
    constructor() {
    ...
    async initialize() {
        ...
        try {
            this._isActive = true;
            this.log(types_js_1.DebugLevel.INFO, 'system', 'Sparky debugging extensions initialized');
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new types_js_1.DebugError(`Failed to initialize debugging extensions: ${errorMessage}`, 'INIT_FAILED');
        }
    }
    /**
    ...
    setDebugLevel(level) {
        (0, index_js_1.ensureSparkyBackend)();
        if (!this._isActive) {
            throw new types_js_1.DebugError('Debugging extensions not initialized', 'NOT_INITIALIZED');
        }
        const oldLevel = this._config.level;
        this._config.level = level;
        this.log(types_js_1.DebugLevel.INFO, 'config', `Debug level changed from ${oldLevel} to ${level}`);
    }
    /**
    ...
    updateDebugConfig(config) {
        ...
        this.log(types_js_1.DebugLevel.INFO, 'config', 'Debug configuration updated');
    }
    /**
    ...
    log(level, category, message, metadata) {
        ...
        if (this.shouldLogToConsole(level)) {
            ...
            switch (level) {
                case types_js_1.DebugLevel.ERROR:
                    console.error(fullMessage, metadata);
                    break;
                case types_js_1.DebugLevel.WARN:
                    console.warn(fullMessage, metadata);
                    break;
                case types_js_1.DebugLevel.INFO:
                    console.info(fullMessage, metadata);
                    break;
                case types_js_1.DebugLevel.DEBUG:
                case types_js_1.DebugLevel.TRACE:
    ...
    clearDebugEntries(level, category) {
        ...
        this.log(types_js_1.DebugLevel.INFO, 'debug', 'Debug entries cleared');
    }
    // ===================================================================
    ...
    traceConstraint(operation, variables, coefficients, sourceLocation) {
        ...
        this.log(types_js_1.DebugLevel.TRACE, 'constraints', `Constraint generated: ${operation}`, {
            variables,
            coefficients,
            sourceLocation
        });
    ...
    async getConstraintDebugInfo() {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.log(types_js_1.DebugLevel.ERROR, 'constraints', `Failed to get constraint debug info: ${errorMessage}`);
            return [];
        }
    }
    /**
    ...
    traceVariable(variableId, context) {
        ...
        this.log(types_js_1.DebugLevel.TRACE, 'variables', `Variable ${variableId} allocated in: ${context}`);
    }
    /**
    ...
    generateDebugReport() {
        ...
        const levels = [types_js_1.DebugLevel.ERROR, types_js_1.DebugLevel.WARN, types_js_1.DebugLevel.INFO];
    ...
    createDefaultConfig() {
        return {
            level: types_js_1.DebugLevel.WARN,
    ...
    shouldLog(level, category) {
        ...
        const levelOrder = [
            types_js_1.DebugLevel.NONE,
            types_js_1.DebugLevel.ERROR,
            types_js_1.DebugLevel.WARN,
            types_js_1.DebugLevel.INFO,
            types_js_1.DebugLevel.DEBUG,
            types_js_1.DebugLevel.TRACE
        ];
    ...
    shouldLogToConsole(level) {
        ...
        return [types_js_1.DebugLevel.ERROR, types_js_1.DebugLevel.WARN, types_js_1.DebugLevel.INFO].includes(level);
    }
}
exports.SparkyDebuggingExtensions = SparkyDebuggingExtensions;
