...
exports.fieldOperations = {
    ...
    compare(bitLength, x, y) {
        try {
            ...
            const less = (0, format_converter_js_1.ensureFieldVar)(runModule.existsOne(() => {
            ...
            const less_or_equal = (0, format_converter_js_1.ensureFieldVar)(runModule.existsOne(() => {
                try {
                    const xVal = getWitnessValue(x);
                    const yVal = getWitnessValue(y);
     ...
     */
    ifThenElse(condition, thenVal, elseVal) {
    ...
    emitIfConstraint(condition, thenVal, elseVal) {
        if (getFieldModule().emitIfConstraint) {
     ...
     */
    emitBooleanAnd(a, b) {
        if (getFieldModule().emitBooleanAnd) {
     ...
     */
    emitBooleanOr(a, b) {
        if (getFieldModule().emitBooleanOr) {
     ...
     */
    emitBooleanNot(a) {
        if (getFieldModule().emitBooleanNot) {
            const result = getFieldModule().emitBooleanNot(a);
            return (0, format_converter_js_1.ensureFieldVar)(result);
        }
 ...
 */
function isZeroFieldVar(fieldVar) {
 ...
 */
function isOneFieldVar(fieldVar) {
    if (!Array.isArray(fieldVar) || fieldVar.length < 2)
        return false;
 ...
 */
function getWitnessValue(fieldVar) {
    if (fieldVar[0] === 0 && Array.isArray(fieldVar[1])) {
        // Constant value
