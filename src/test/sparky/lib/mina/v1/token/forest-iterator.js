...
exports.TokenAccountUpdateIterator = void 0;
...
const AccountUpdateIterator = merkle_list_js_1.MerkleListIterator.create(account_update_js_1.AccountUpdateForest.prototype.innerProvable, account_update_js_1.AccountUpdateForest._nextHash, account_update_js_1.AccountUpdateForest.emptyHash);
class Layer extends (0, struct_js_1.Struct)({
    forest: AccountUpdateIterator,
    mayUseToken: account_update_js_1.AccountUpdate.MayUseToken.type,
}) {
}
const ParentLayers = merkle_list_js_1.MerkleList.create(Layer);
const MayUseToken = account_update_js_1.AccountUpdate.MayUseToken;
/**
...
class TokenAccountUpdateIterator {
    ...
    next() {
        ...
        let canAccessThisToken = provable_js_1.Provable.equal(MayUseToken.type, update.body.mayUseToken, this.currentLayer.mayUseToken);
        let isSelf = account_update_js_1.TokenId.derive(update.publicKey, update.tokenId).equals(this.selfToken);
        ...
        this.currentLayer = provable_js_1.Provable.if(childLayerFinished, Layer, currentOrParentLayer, childLayer);
        return { accountUpdate: update, usesThisToken };
    }
    assertFinished(message) {
        (0, common_js_1.assert)(this.currentLayer.forest.isAtStart(), message ?? 'TokenAccountUpdateIterator not finished');
    }
}
exports.TokenAccountUpdateIterator = TokenAccountUpdateIterator;
