...
const FieldVar = {
    ...
    add(x, y) {
        ...
        if (FieldVar.isConstant(x) && FieldVar.isConstant(y)) {
            return FieldVar.constant(finite_field_js_1.Fp.add(x[1][1], y[1][1]));
        }
        return [FieldType.Add, x, y];
    },
    scale(c, x) {
        ...
        if (FieldVar.isConstant(x)) {
            return FieldVar.constant(finite_field_js_1.Fp.mul(c0[1], x[1][1]));
        }
        if (FieldVar.isScale(x)) {
            return [FieldType.Scale, FieldConst.fromBigint(finite_field_js_1.Fp.mul(c0[1], x[1][1])), x[2]];
        }
