...
exports.createBytes = exports.Bytes = void 0;
...
class Bytes {
    ...
    constructor(bytes) {
        ...
        (0, common_js_1.assert)(bytes.length <= size, `Expected at most ${size} bytes, got ${bytes.length}`);
        // pad the data with zeros
        let padding = Array.from({ length: size - bytes.length }, () => new int_js_1.UInt8(0));
    ...
    static from(data) {
        ...
        return new this([...data].map(int_js_1.UInt8.from));
    }
    toBytes() {
     ...
     */
    static random() {
        let bytes = (0, random_js_1.randomBytes)(this.size);
        return this.from(bytes);
    }
    /**
     ...
     */
    static fromHex(xs) {
        let bytes = (0, arrays_js_1.chunkString)(xs, 2).map((s) => parseInt(s, 16));
        return this.from(bytes);
    }
    /**
    ...
    base64Encode() {
        ...
        plainBits.push(...Array(bitPadding).fill(new bool_js_1.Bool(false)));
        let encodedBytes = [];
        ...
        for (let i = 0; i < plainBits.length; i += 6) {
            ...
            const indexTableByte = int_js_1.UInt8.Unsafe.fromField(field_js_1.Field.fromBits(byteBits));
        ...
        encodedBytes.push(...Array(paddingLength).fill(int_js_1.UInt8.from(61)));
    ...
    base64Decode(byteLength) {
        ...
        (0, common_js_1.assert)(charLength % 4 === 0, 'Input base64 byte length should be a multiple of 4!');
        let decodedB64Bytes = new Array(byteLength).fill(int_js_1.UInt8.from(0));
        ...
        for (let i = 0; i < charLength / 4; i++) {
            ...
            for (let j = 0; j < 3; j++) {
                if (idx + j < byteLength) {
                    decodedB64Bytes[idx + j] = int_js_1.UInt8.Unsafe.fromField(field_js_1.Field.fromBits(bitsOut[i][j]));
                }
            }
            idx += 3;
        }
     ...
     */
    chunk(size) {
        return (0, arrays_js_1.chunk)(this.bytes, size);
    }
     ...
     */
    static get size() {
        (0, common_js_1.assert)(this._size !== undefined, 'Bytes not initialized');
        return this._size;
    }
    get length() {
     ...
     */
    static get provable() {
        (0, common_js_1.assert)(this._provable !== undefined, 'Bytes not initialized');
...
function createBytes(size) {
    return class Bytes_ extends Bytes {
        static _size = size;
        static _provable = (0, provable_derivers_js_1.provableFromClass)(Bytes_, {
            bytes: provable_js_1.Provable.Array(int_js_1.UInt8, size),
        });
    };
}
...
function base64DecodeLookup(input) {
    ...
    let isValidBase64Chars = new field_js_1.Field(0);
    // ['A' - 'Z'] range
...
function base64EncodeLookup(input) {
    ...
    return int_js_1.UInt8.Unsafe.fromField(sum_slash);
}
