...
class Bool {
    ...
    constructor(x) {
        ...
        this.value = fieldvar_js_1.FieldVar.constant(BigInt(x));
    }
    isConstant() {
    ...
    not() {
        ...
        let not = new field_js_1.Field(1).sub(this.toField());
    ...
    equals(y) {
        ...
        // 2x * y === x + y - z
        // return 1 - z
        let z = (0, exists_js_1.existsOne)(() => BigInt(this.toBoolean() !== toBoolean(y)));
        let x = this.toField();
        let y_ = Bool.toField(y);
        (0, compatible_js_1.assertMul)(x.add(x), y_, x.add(y_).sub(z));
    ...
    toBoolean() {
        if (this.isConstant()) {
            return fieldvar_js_1.FieldConst.equal(this.value[1], fieldvar_js_1.FieldConst[1]);
        }
        if (!bindings_js_1.Snarky.run.inProverBlock()) {
            throw Error((0, field_js_1.readVarMessage)('toBoolean', 'b', 'Bool'));
        }
        let value = bindings_js_1.Snarky.field.readVar(this.value);
        return fieldvar_js_1.FieldConst.equal(value, fieldvar_js_1.FieldConst[1]);
    }
    static toField(x) {
        return new field_js_1.Field(toFieldVar(x));
    }
    /**
     * Boolean negation.
     */
...
exports.Bool = Bool;
(0, field_constructor_js_1.setBoolConstructor)(Bool);
const BoolBinable = (0, binable_js_1.defineBinable)({
    toBytes(b) {
...
function toFieldVar(x) {
    if (x instanceof Bool)
        return x.value;
    return fieldvar_js_1.FieldVar.constant(BigInt(x));
}
