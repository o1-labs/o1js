...
exports.sliceField3 = exports.bytesToWords = exports.wordsToBytes = exports.wordToBytes = exports.bytesToWord = void 0;
/**
...
function wordToBytes(word, bytesPerWord = 8) {
    let bytes = provable_js_1.Provable.witness(provable_js_1.Provable.Array(int_js_1.UInt8, bytesPerWord), () => {
        let w = word.toBigInt();
        return Array.from({ length: bytesPerWord }, (_, k) => int_js_1.UInt8.from((w >> BigInt(8 * k)) & 0xffn));
    });
    // check decomposition
...
function bytesToWords(bytes, bytesPerWord = 8) {
    return (0, arrays_js_1.chunk)(bytes, bytesPerWord).map(bytesToWord);
}
...
function sliceField3([x0, x1, x2], { maxBits, chunkSize }) {
    ...
    (0, common_js_1.assert)(maxBits <= 3 * l_, `expected max bits <= 3*${l_}, got ${maxBits}`);
    // first limb
...
exports.sliceField3 = sliceField3;
/**
...
function sliceField(x, maxBits, chunkSize, leftover) {
    let bits = (0, exists_js_1.exists)(maxBits, () => {
        let bits = (0, bigint_helpers_js_1.bigIntToBits)(x.toBigInt());
        // normalize length
    ...
    let chunks = [];
    let sum = field_js_1.Field.from(0n);
    ...
    if (leftover !== undefined) {
        ...
        let remainingChunk = field_js_1.Field.from(0n);
    ...
    for (; i < maxBits; i += chunkSize) {
        ...
        let chunk = field_js_1.Field.from(0n);
