...
const SHA2 = {
    ...
    hash(length, data) {
        ...
        let digest = wrapped_classes_js_1.Bytes.from(H.map((x) => x.toBytesBE()).flat());
    ...
    padding,
    initialState(length) {
        return SHA2Constants.H[length].map((x) => isUInt32(length) ? int_js_1.UInt32.from(x) : int_js_1.UInt64.from(x));
    },
};
exports.SHA2 = SHA2;
/**
...
function padding(length, data) {
    ...
    let message = wrapped_classes_js_1.Bytes.from(data);
    ...
    let k = Number((0, finite_field_js_1.mod)(paddingValue - (BigInt(l) + 1n), blockLength));
    ...
    let padding = paddingBits.map((x) => int_js_1.UInt8.from(BigInt('0b' + x)));
    ...
    let chunks = isUInt32(length)
        ? createChunks(paddedMessage, 4, int_js_1.UInt32.fromBytesBE)
        : createChunks(paddedMessage, 8, int_js_1.UInt64.fromBytesBE);
    ...
    return (0, arrays_js_1.chunk)(chunks, 16);
}
/**
...
function reduceMod(length, x) {
    return isUInt32(length)
        ? int_js_1.UInt32.Unsafe.fromField((0, arithmetic_js_1.divMod32)(x, 32 + 16).remainder)
        : int_js_1.UInt64.Unsafe.fromField((0, arithmetic_js_1.divMod64)(x, 64 + 16).remainder);
}
...
function Ch(length, x, y, z) {
    ...
    if (isUInt32(length)) {
        ...
        return int_js_1.UInt32.Unsafe.fromField(ch);
    }
    else {
        let xAndY = x.and(y).value;
        let xNotAndZ = x.not().and(z).value;
        let ch = xAndY.add(xNotAndZ).seal();
        return int_js_1.UInt64.Unsafe.fromField(ch);
    }
}
function Maj(length, x, y, z) {
    ...
    if (isUInt32(length)) {
        ...
        return int_js_1.UInt32.Unsafe.fromField(maj);
    }
    else {
        let sum = x.value.add(y.value).add(z.value).seal();
        let xor = x.xor(y).xor(z).value;
        let maj = sum.sub(xor).div(2).seal();
        return int_js_1.UInt64.Unsafe.fromField(maj);
    }
}
...
function sigma(length, u, bits, firstShifted = false) {
    ...
    let [x0, x1, x2, x3] = (0, exists_js_1.exists)(4, () => {
        let xx = x.toBigInt();
        return [(0, common_js_1.bitSlice)(xx, 0, d0), (0, common_js_1.bitSlice)(xx, r0, d1), (0, common_js_1.bitSlice)(xx, r1, d2), (0, common_js_1.bitSlice)(xx, r2, d3)];
    });
    ...
    (0, range_check_js_1.rangeCheck16)(x0);
    (0, range_check_js_1.rangeCheck16)(x1);
    (0, range_check_js_1.rangeCheck16)(x2);
    (0, range_check_js_1.rangeCheck16)(x3);
    ...
    if (!firstShifted) {
        ...
        (0, range_check_js_1.rangeCheck16)(x0.mul(1 << (16 - d0)).seal());
    }
    ...
    return int_js_1.UInt32.Unsafe.fromField(xRotR0)
        .xor(int_js_1.UInt32.Unsafe.fromField(xRotR1))
        .xor(int_js_1.UInt32.Unsafe.fromField(xRotR2));
}
