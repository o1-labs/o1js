...
class Sum {
    ...
    get result() {
        (0, common_js_1.assert)(this.#result !== undefined, 'sum not finished');
        return this.#result;
    }
    get length() {
    ...
    add(y) {
        (0, common_js_1.assert)(this.#result === undefined, 'sum already finished');
        this.#ops.push(1n);
    ...
    finish(f, isChained = false) {
        ...
        let n = signs.length;
        if (n === 0)
            return this.#return(this.#summands[0]);
        // constant case
        if (this.isConstant()) {
            return this.#return(sum(this.#summands, signs, f));
        }
        // provable case
        let x = this.#summands.map(common_js_1.toVars);
        let result = x[0];
        for (let i = 0; i < n; i++) {
    ...
    finishForMulInput(f, isChained = false) {
        ...
        // provable case
        let xs = this.#summands.map(common_js_1.toVars);
    ...
    rangeCheck() {
        ...
        if (this.#ops.length > 0)
            (0, range_check_js_1.multiRangeCheck)(this.#result);
    }
    static fromUnfinished(x) {
        if (x instanceof Sum) {
            (0, common_js_1.assert)(x.#result === undefined, 'sum already finished');
            return x;
        }
        return new Sum(x);
    }
