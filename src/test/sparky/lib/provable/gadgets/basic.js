...
exports.ScaledVar = exports.bilinear = exports.linear = exports.emptyCell = exports.toLinearCombination = exports.reduceToScaledVarSparky = exports.reduceToScaledVar = exports.arrayGetGeneric = exports.assertNotVectorEquals = exports.assertOneOf = exports.arrayGet = exports.assertBilinear = exports.assertMul = void 0;
/**
...
function arrayGet(array, index) {
    ...
    if (index.isConstant()) {
        return array[Number(index.toBigInt())] ?? (0, field_constructor_js_1.createField)(0n);
    }
    let i = (0, common_js_1.toVar)(index);
    // witness result
    let a = (0, exists_js_1.existsOne)(() => array[Number(i.toBigInt())]?.toBigInt() ?? 0n);
    ...
    let n = array.length;
    for (let j = 0; j < n; j++) {
        let zj = (0, exists_js_1.existsOne)(() => {
            let zj = finite_field_js_1.Fp.div(finite_field_js_1.Fp.sub(a.toBigInt(), array[j].toBigInt()), finite_field_js_1.Fp.sub(i.toBigInt(), finite_field_js_1.Fp.fromNumber(j)));
            return zj ?? 0n;
        });
        ...
        if (array[j].isConstant()) {
            ...
            let aMinusAj = (0, common_js_1.toVar)(a.sub(array[j]));
...
function assertNotVectorEquals(x, c) {
    let xv = x.map(common_js_1.toVar);
    let n = c.length;
    (0, assert_js_1.assert)(n > 1 && x.length === n, 'vector lengths must match');
    // witness vector z
    let z = (0, exists_js_1.exists)(n, () => {
        ...
        if (i === -1)
            return z;
        // z[i] = (x[i] - c[i])^-1
        z[i] = finite_field_js_1.Fp.inverse(finite_field_js_1.Fp.sub(x[i].toBigInt(), c[i])) ?? 0n;
        return z;
    });
...
exports.assertNotVectorEquals = assertNotVectorEquals;
...
function linear(x, [a, b]) {
    let z = (0, exists_js_1.existsOne)(() => {
        let x0 = (0, field_constructor_js_1.createField)(x).toBigInt();
        return a * x0 + b;
    });
    // a*x - z + b === 0
    gates_js_1.Gates.generic({ left: a, right: 0n, out: -1n, mul: 0n, const: b }, { left: x, right: emptyCell(), out: z });
    return z;
}
...
function bilinear(x, y, [a, b, c, d]) {
    let z = (0, exists_js_1.existsOne)(() => {
        let x0 = (0, field_constructor_js_1.createField)(x).toBigInt();
        let y0 = (0, field_constructor_js_1.createField)(y).toBigInt();
        return a * x0 * y0 + b * x0 + c * y0 + d;
    });
    // b*x + c*y - z + a*x*y + d === 0
    gates_js_1.Gates.generic({ left: b, right: c, out: -1n, mul: a, const: d }, { left: x, right: y, out: z });
    return z;
}
...
function assertBilinear(x, y, [a, b, c, d], z) {
    // b*x + c*y - z? + a*x*y + d === 0
    gates_js_1.Gates.generic({ left: b, right: c, out: z === undefined ? 0n : -1n, mul: a, const: d }, { left: x, right: y, out: z === undefined ? emptyCell() : z });
}
exports.assertBilinear = assertBilinear;
function emptyCell() {
    return (0, exists_js_1.existsOne)(() => 0n);
}
...
function reduceToScaledVar(x) {
    let { constant: c, terms } = toLinearCombination((0, gates_js_1.fieldVar)(x));
    // sort terms alphabetically by variable index
    // (to match snarky implementation)
    terms.sort(([, [, i]], [, [, j]]) => i - j);
    if (terms.length === 0) {
        // constant
        return [fieldvar_js_1.FieldType.Constant, fieldvar_js_1.FieldConst.fromBigint(c)];
    }
    if (terms.length === 1) {
        let [s, x] = terms[0];
        if (c === 0n) {
            // s*x
            return [fieldvar_js_1.FieldType.Scale, fieldvar_js_1.FieldConst.fromBigint(s), x];
        }
        else {
            // res = s*x + c
            let res = linear(x, [s, c]);
            return [fieldvar_js_1.FieldType.Scale, fieldvar_js_1.FieldConst[1], res.value];
        }
    }
...
exports.reduceToScaledVar = reduceToScaledVar;
// Sparky-specific function for multi-term optimization
function reduceToScaledVarSparky(x) {
    let { constant: c, terms } = toLinearCombination((0, gates_js_1.fieldVar)(x));
    // sort terms alphabetically by variable index
    terms.sort(([, [, i]], [, [, j]]) => i - j);
    if (terms.length === 0) {
...
exports.reduceToScaledVarSparky = reduceToScaledVarSparky;
/**
...
function toLinearCombination(x, sx = 1n, lincom = {
    ...
    if (sx === 0n)
        return lincom;
    switch (x[0]) {
        case fieldvar_js_1.FieldType.Constant: {
            // a constant is added to the constant term
            let [, [, c]] = x;
            return { constant: finite_field_js_1.Fp.add(constant, finite_field_js_1.Fp.mul(sx, c)), terms };
        }
        case fieldvar_js_1.FieldType.Var: {
            ...
            let [sy] = y;
            y[0] = finite_field_js_1.Fp.add(sy, sx);
            if (y[0] === 0n) {
        ...
        case fieldvar_js_1.FieldType.Scale: {
            // sx * (s * x) + ... = (sx * s) * x + ...
            let [, [, s], v] = x;
            return toLinearCombination(v, finite_field_js_1.Fp.mul(sx, s), lincom);
        }
        case fieldvar_js_1.FieldType.Add: {
...
exports.toLinearCombination = toLinearCombination;
function isVar(x) {
    return x[0] === fieldvar_js_1.FieldType.Scale;
}
function isConst(x) {
    return x[0] === fieldvar_js_1.FieldType.Constant;
}
function getVar(x) {
    return [x[1][1], x[2]];
}
...
function arrayGetGeneric(type, array, index) {
    ...
    // witness result
    let a = provable_js_1.Provable.witness(type, () => array[Number(index)]);
...
exports.arrayGetGeneric = arrayGetGeneric;
