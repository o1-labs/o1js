...
exports.ForeignCurveNotNeeded = exports.toPoint = exports.ForeignCurve = exports.createForeignCurve = void 0;
const elliptic_curve_js_1 = require("../../../bindings/crypto/elliptic-curve.js");
const foreign_field_js_1 = require("../foreign-field.js");
const elliptic_curve_js_2 = require("../gadgets/elliptic-curve.js");
...
class ForeignCurve {
    ...
    static fromHex(hex) {
        ...
        const bytes = bytes_js_1.Bytes.fromHex(hex).toBytes();
        ...
        const x = BigInt('0x' + bytes_js_1.Bytes.from(xBytes).toHex());
        const y = BigInt('0x' + bytes_js_1.Bytes.from(yBytes).toHex());
    ...
    static fromEthers(hex) {
        ...
        const bytes = bytes_js_1.Bytes.fromHex(hex).toBytes(); // convert hex string to Uint8Array
        ...
        const x = BigInt('0x' + bytes_js_1.Bytes.from(xBytes).toHex()); // convert Uint8Array to bigint
        let p = undefined;
        ...
        if (len === 33 && [0x02, 0x03].includes(head)) {
            ...
            (0, common_js_1.assert)(0n < x && x < this.Bigint.Field.modulus);
        ...
        if (len === 65 && head === 0x04) {
            ...
            p = { x, y: BigInt('0x' + bytes_js_1.Bytes.from(yBytes).toHex()) };
        }
     ...
     */
    isConstant() {
        return provable_js_1.Provable.isConstant(this.Constructor, this);
    }
    /**
    ...
    add(h) {
        ...
        let p = elliptic_curve_js_2.EllipticCurve.add(toPoint(this), toPoint(h_), Curve);
    ...
    double() {
        let Curve = this.Constructor.Bigint;
        let p = elliptic_curve_js_2.EllipticCurve.double(toPoint(this), Curve);
        return new this.Constructor(p);
    }
    /**
    ...
    scale(scalar) {
        ...
        let p = elliptic_curve_js_2.EllipticCurve.scale(scalar_.value, toPoint(this), Curve);
        return new this.Constructor(p);
    }
    static assertOnCurve(g) {
        elliptic_curve_js_2.EllipticCurve.assertOnCurve(toPoint(g), this.Bigint);
    }
    /**
    ...
    static assertInSubgroup(g) {
        if (this.Bigint.hasCofactor) {
            elliptic_curve_js_2.EllipticCurve.assertInSubgroup(toPoint(g), this.Bigint);
        }
    }
    /**
     ...
     */
    static check(g) {
        (0, range_check_js_1.multiRangeCheck)(g.x.value);
        (0, range_check_js_1.multiRangeCheck)(g.y.value);
     ...
     */
    static get Bigint() {
        (0, common_js_1.assert)(this._Bigint !== undefined, 'ForeignCurve not initialized');
        return this._Bigint;
    }
    /**
     ...
     */
    static get Field() {
        (0, common_js_1.assert)(this._Field !== undefined, 'ForeignCurve not initialized');
        return this._Field;
    }
    /**
     ...
     */
    static get Scalar() {
        (0, common_js_1.assert)(this._Scalar !== undefined, 'ForeignCurve not initialized');
        return this._Scalar;
    }
    /**
     ...
     */
    static get provable() {
        (0, common_js_1.assert)(this._provable !== undefined, 'ForeignCurve not initialized');
        return this._provable;
    }
}
exports.ForeignCurve = ForeignCurve;
/**
...
exports.ForeignCurveNotNeeded = ForeignCurveNotNeeded;
/**
...
function createForeignCurve(params) {
    (0, common_js_1.assert)(params.modulus > range_check_js_1.l2Mask + 1n, 'Base field moduli smaller than 2^176 are not supported');
    const FieldUnreduced = (0, foreign_field_js_1.createForeignField)(params.modulus);
    const ScalarUnreduced = (0, foreign_field_js_1.createForeignField)(params.order);
    class Field extends FieldUnreduced.AlmostReduced {
    }
    class Scalar extends ScalarUnreduced.AlmostReduced {
    }
    const BigintCurve = (0, elliptic_curve_js_1.createCurveAffine)(params);
    class Curve extends ForeignCurve {
        static _Bigint = BigintCurve;
        static _Field = Field;
        static _Scalar = Scalar;
        static _provable = (0, provable_derivers_js_1.provableFromClass)(Curve, { x: Field, y: Field });
    }
    return Curve;
}
exports.createForeignCurve = createForeignCurve;
