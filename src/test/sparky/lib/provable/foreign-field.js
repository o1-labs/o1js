...
exports.createForeignField = void 0;
...
class ForeignField {
    static _Bigint = undefined;
    static _modulus = undefined;
    // static parameters
    static get Bigint() {
        (0, common_js_1.assert)(this._Bigint !== undefined, 'ForeignField class not initialized.');
        return this._Bigint;
    }
    static get modulus() {
        (0, common_js_1.assert)(this._modulus !== undefined, 'ForeignField class not initialized.');
    ...
    static get Unreduced() {
        (0, common_js_1.assert)(this._variants !== undefined, 'ForeignField class not initialized.');
    ...
    constructor(x) {
        ...
        // constant
        this.value = foreign_field_js_1.Field3.from((0, finite_field_js_1.mod)(BigInt(x), p));
    }
    static from(x) {
     ...
     */
    isConstant() {
        return foreign_field_js_1.Field3.isConstant(this.value);
    }
    /**
     * @internal
     ...
     */
    toConstant() {
        let constantLimbs = types_js_1.Tuple.map(this.value, (l) => l.toConstant());
     ...
     */
    toBigInt() {
        return foreign_field_js_1.Field3.toBigint(this.value);
    }
    /**
    ...
    static assertAlmostReduced(...xs) {
        gadgets_js_1.Gadgets.ForeignField.assertAlmostReduced(xs.map((x) => x.value), this.modulus, { skipMrc: true });
        return types_js_1.Tuple.map(xs, this.AlmostReduced.unsafeFrom);
    }
    /**
    ...
    neg() {
        ...
        let xNeg = gadgets_js_1.Gadgets.ForeignField.neg(this.value, this.modulus);
    ...
    static sum(xs, operations) {
        ...
        let z = gadgets_js_1.Gadgets.ForeignField.sum(fields, ops, p);
    ...
    assertEquals(y, message) {
        ...
        try {
            if (this.isConstant() && isConstant(y)) {
                let x = this.toBigInt();
                let y0 = (0, finite_field_js_1.mod)(toBigInt(y), p);
                if (x !== y0) {
                    throw Error(`ForeignField.assertEquals(): ${x} != ${y0}`);
                }
                return new this.Constructor.Canonical(this.value);
            }
            provable_js_1.Provable.assertEqual(this.Constructor, this, new this.Constructor(y));
    ...
    assertLessThan(c, message) {
        (0, common_js_1.assert)(c >= 0 && c < 1n << range_check_js_1.l3, `ForeignField.assertLessThan(): expected c <= c < 2^264, got ${c}`);
        try {
            gadgets_js_1.Gadgets.ForeignField.assertLessThan(this.value, toBigInt(c));
        }
        catch (err) {
            throw (0, field_js_1.withMessage)(err, message);
        }
    ...
    toBits(length) {
        ...
        if (length === undefined)
            length = sizeInBits;
        (0, field_js_1.checkBitLength)('ForeignField.toBits()', length, sizeInBits);
    ...
    static fromBits(bits) {
        ...
        (0, field_js_1.checkBitLength)('ForeignField.fromBits()', length, this.sizeInBits);
        let limbSize = Number(range_check_js_1.l);
        let l0 = field_js_1.Field.fromBits(bits.slice(0 * limbSize, 1 * limbSize));
        let l1 = field_js_1.Field.fromBits(bits.slice(1 * limbSize, 2 * limbSize));
        let l2 = field_js_1.Field.fromBits(bits.slice(2 * limbSize, 3 * limbSize));
    ...
    static _provable = undefined;
    /**
     * `Provable<ForeignField>`, see {@link Provable}
     */
    static get provable() {
        (0, common_js_1.assert)(this._provable !== undefined, 'ForeignField class not initialized.');
...
class ForeignFieldWithMul extends ForeignField {
    ...
    mul(y) {
        const p = this.modulus;
        let z = gadgets_js_1.Gadgets.ForeignField.mul(this.value, toLimbs(y, p), p);
    ...
    inv() {
        const p = this.modulus;
        let z = gadgets_js_1.Gadgets.ForeignField.inv(this.value, p);
    ...
    div(y) {
        const p = this.modulus;
        let z = gadgets_js_1.Gadgets.ForeignField.div(this.value, toLimbs(y, p), p);
        return new this.Constructor.AlmostReduced(z);
    }
}
class UnreducedForeignField extends ForeignField {
    ...
    static check(x) {
        gadgets_js_1.Gadgets.multiRangeCheck(x.value);
    }
}
class AlmostForeignField extends ForeignFieldWithMul {
     ...
     */
    equals(y) {
        return foreign_field_js_1.ForeignField.equals(this.value, BigInt(y), this.modulus);
    }
}
class CanonicalForeignField extends ForeignFieldWithMul {
    ...
    equals(y) {
        ...
        let x01 = x0.add(x1.mul(1n << range_check_js_1.l)).seal();
        let y01 = y0.add(y1.mul(1n << range_check_js_1.l)).seal();
...
function toLimbs(x, p) {
    if (x instanceof ForeignField)
        return x.value;
    return foreign_field_js_1.Field3.from((0, finite_field_js_1.mod)(BigInt(x), p));
}
function toBigInt(x) {
    if (x instanceof ForeignField)
        return x.toBigInt();
...
function createForeignField(modulus) {
    (0, common_js_1.assert)(modulus > 0n, `ForeignField: modulus must be positive, got ${modulus}`);
    (0, common_js_1.assert)(modulus < foreignFieldMax, `ForeignField: modulus exceeds the max supported size of 2^${foreignFieldMaxBits}`);
    let Bigint = (0, finite_field_js_1.createField)(modulus);
...
exports.createForeignField = createForeignField;
...
const foreignFieldMaxBits = (BigInt(finite_field_js_1.Fp.sizeInBits - 1) + 3n * range_check_js_1.l) / 2n;
...
function provable(Class) {
    return {
        ...
        fromFields(fields) {
            let limbs = types_js_1.TupleN.fromArray(3, fields);
        ...
        toCanonical(x) {
            if (x.type === 'FullyReduced')
                return x;
            return new Class(foreign_field_js_1.ForeignField.toCanonical(x.value, x.modulus));
        },
        toValue(x) {
            return x.toBigInt();
        },
        ...
        toInput(x) {
            let l_ = Number(range_check_js_1.l);
            return {
                packed: [
                    [x.value[0], l_],
                    [x.value[1], l_],
