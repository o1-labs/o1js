/**
 * Test to analyze Bool constraint generation in Sparky vs Snarky
 */

import { Field, Bool, Provable } from '../index.js';
...
async function analyzeBooleanConstraints() {
  ...
  for (const backend of backends) {
    ...
    try {
      ...
      // Test 5: Examine the individual steps in a.or(b) = a.not().and(b.not()).not()
      console.log('\n📊 Test 5: Decomposed OR operation steps');
      const decomposedOrConstraints = await Provable.constraintSystem(() => {
        const a = Provable.witness(Bool, () => Bool(true));
        const b = Provable.witness(Bool, () => Bool(false));
        
        // Step-by-step decomposition of a.or(b)
        const notA = a.not();                    // Step 1: NOT a
        const notB = b.not();                    // Step 2: NOT b  
        const andNotANotB = notA.and(notB);      // Step 3: (NOT a) AND (NOT b)
        const result = andNotANotB.not();       // Step 4: NOT((NOT a) AND (NOT b))
        
        result.assertEquals(result);
        return result;
      });
      
      console.log(`   Constraints: ${decomposedOrConstraints.gates.length}`);
      console.log(`   Gate types: ${decomposedOrConstraints.gates.map(g => g.type).join(', ')}`);
