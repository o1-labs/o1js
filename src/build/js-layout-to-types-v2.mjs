...
function analyzeTypeDependencies(layout) {
  ...
  function addDeps(node) {
    if(node.type === 'object') {
      ...
      const nodeDeps = node.keys
        .map((key) => castDep(node.entries[key]))
        .filter((x) => !!x);
  ...
  if('checkedType' in node && !ignoredCheckedTypes.has(node.name)) {
    ...
    switch(node.type) {
      ...
      case 'array': {
        ...
        const len = 'staticLength' in node && !!node.staticLength ? node.staticLength.toString() : 'null';
      ...
      case 'option': {
        switch(node.optionType) {
          case 'orUndefined':
            return `new BindingsType.Option.OrUndefined<${renderJsType(node.inner)}>(${renderLayoutType(node.inner)})`;
          case 'flaggedOption':
          ...
          case 'closedInterval':
            ...
            assert.deepEqual(node.inner.keys, ['lower', 'upper']);
            assert.deepEqual(node.inner.entries.lower, node.inner.entries.upper);
            return `new BindingsType.Option.ClosedInterval(${renderLayoutType(node.inner.entries.lower)})`;
        }
      }
      default:
        throw new Error(`unhandled layout node type: ${node.type}`);
