...
#
# 1. Identifies the latest version number in the CHANGELOG (following semantic
# versioning).
...
#    - Adds a new entry for the upcoming release using the incremented version
#    number and the current date.
#    - Updates the link for the [Unreleased] section to point from the current
#    commit to HEAD.
#
# Usage:
# It should be run in the root directory of the repository where the
# CHANGELOG.md is located.
# Ensure that you have the necessary permissions to commit and push changes to
# the repository.
...
# Step 3: Capture the current git commit and truncate it to the first 9
# characters
