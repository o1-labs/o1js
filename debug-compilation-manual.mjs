...
/**
 * Manual debug script for SmartContract compilation issues
 * Created: January 5, 2025 05:45 UTC
 * 
 * This script manually calls the decorator functions like the compiled TypeScript does.
 */
...
async function testBasicSmartContractCompilation(backend) {
  ...
  try {
    ...
    const { SmartContract, State, state, Field: DynamicField, method, Mina, switchBackend, getCurrentBackend, declareMethods } = o1js;
    ...
    // Apply decorators using declareMethods (the JavaScript alternative)
    console.log('   Applying decorators using declareMethods...');
    
    // Apply @state decorator properly
    console.log('   Applying @state decorator...');
    const stateDecorator = state(DynamicField);
    stateDecorator(TestContract.prototype, 'value', undefined);
    
    // Apply @method decorators using declareMethods
    console.log('   Applying @method decorators...');
    declareMethods(TestContract, {
      increment: [],      // no arguments
      multiply: [DynamicField, DynamicField]  // two Field arguments
    });
    
    console.log('   ✅ SmartContract class defined and decorated');
