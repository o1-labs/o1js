...
When testing native backend changes locally, refresh the installed package after
building. For example, on macOS arm64:

```sh
npm run build:native
cp native/darwin-arm64/kimchi_napi.node node_modules/@o1js/native-darwin-arm64/kimchi_napi.node
```

Alternatively, reinstall dependencies after rebuilding the native package so npm
refreshes the local `file:` dependency.
