...
import { switchBackend, getCurrentBackend, SmartContract, State, state, Field, method, Mina } from './dist/node/index.js';
...
async function testBasicSmartContractCompilation(backend) {
  ...
  try {
    ...
    // Get o1js after backend switch - modules should already be loaded
    console.log('\n2. Verifying o1js modules...');
    console.log('   SmartContract available:', typeof SmartContract);
    console.log('   State available:', typeof State);
    console.log('   state available:', typeof state);
    console.log('   Field available:', typeof Field);
    console.log('   method available:', typeof method);
    console.log('   Mina available:', typeof Mina);
    console.log('   ✅ o1js modules verified');
    ...
    // Store the original methods before decorating
    const originalUpdate = SimpleContract.prototype.update;
    const originalMultiply = SimpleContract.prototype.multiply;
    
    // Apply method decorators
    SimpleContract.prototype.update = method(originalUpdate);
    SimpleContract.prototype.multiply = method(originalMultiply);
