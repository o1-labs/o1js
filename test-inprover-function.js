...
import { Field, Bool, switchBackend } from './dist/node/index.js';

console.log('Testing inProver function change...\n');
...
const snarkyField = Field;
try {
  const inProverResult = snarkyField.inProver();
  console.log(`  inProver() = ${inProverResult}`);
...
const witnessResult = Field.witness(() => {
  const result = snarkyField.inProver();
...
const sparkyField = Field;
try {
  const inProverResult = sparkyField.inProver();
  console.log(`  inProver() = ${inProverResult}`);
  console.log('  ✓ inProver() works as function\n');
} catch (e) {
  console.error('  ✗ Error calling inProver():', e.message);
...
const sparkyWitnessResult = Field.witness(() => {
  const result = sparkyField.inProver();
  console.log(`  Inside witness: inProver() = ${result}`);
  return result;
});
