...
exports.unsetSrsCache = exports.setSrsCache = exports.srs = void 0;
const cache_js_1 = require("../../../lib/proof-system/cache.js");
...
exports.setSrsCache = setSrsCache;
function unsetSrsCache() {
    cache = undefined;
}
exports.unsetSrsCache = unsetSrsCache;
const srsVersion = 1;
function cacheHeaderLagrange(f, domainSize) {
    let id = `lagrange-basis-${f}-${domainSize}`;
    return (0, cache_js_1.withVersion)({
        kind: 'lagrange-basis',
        persistentId: id,
        uniqueId: id,
        dataType: 'string',
        ...
        create(size) {
            ...
            if (srs === undefined) {
                if (cache === undefined) {
                    ...
                    srs = (0, cache_js_1.readCache)(cache, header, (bytes) => {
                        ...
                        let mlSrs = base_js_1.MlArray.mapTo(jsonSrs, curve_js_1.OrInfinity.fromJSON);
                    ...
                    if (srs === undefined) {
                        ...
                        if (cache.canWrite) {
                            ...
                            let jsonSrs = base_js_1.MlArray.mapFrom(mlSrs, curve_js_1.OrInfinity.toJSON);
                            let bytes = new TextEncoder().encode(JSON.stringify(jsonSrs));
                            (0, cache_js_1.writeCache)(cache, header, bytes);
                        }
                    }
                }
                srsStore[f][size] = srs;
        ...
        lagrangeCommitment(srs, domainSize, i) {
            ...
            if (commitment === undefined) {
                if (cache === undefined) {
                    ...
                    let didRead = (0, cache_js_1.readCache)(cache, header, (bytes) => {
                    ...
                    (0, errors_js_1.assert)(c !== undefined, 'commitment exists after setting');
                    commitment = c;
                }
            }
...
function polyCommsToJSON(comms) {
    return base_js_1.MlArray.mapFrom(comms, ([, elems]) => {
        return {
            shifted: base_js_1.MlArray.mapFrom(elems, curve_js_1.OrInfinity.toJSON),
            unshifted: undefined,
        };
    });
}
function polyCommsFromJSON(json) {
    return base_js_1.MlArray.mapTo(json, ({ shifted, unshifted }) => {
        return [0, base_js_1.MlArray.mapTo(shifted, curve_js_1.OrInfinity.fromJSON)];
    });
}
