...
exports.Bijective = exports.bytesFromFields = exports.bytesToFields = exports.stringFromFields = exports.stringToFields = void 0;
...
exports.stringToFields = stringToFields;
/**
...
exports.stringFromFields = stringFromFields;
const STOP = 0x01;
/**
...
function bytesToFields(bytes) {
    ...
    for (let byte of bytes) {
        ...
        bitPosition += 8n;
        if (bitPosition === 248n) {
            fields.push((0, wrapped_js_1.Field)(currentBigInt.toString()));
            currentBigInt = 0n;
            bitPosition = 0n;
        }
    }
...
exports.bytesToFields = bytesToFields;
/**
...
exports.bytesFromFields = bytesFromFields;
...
exports.Bijective = Bijective;
function toBytesBijective(fields, p) {
    let fieldsBigInts = fields.map((x) => x.toBigInt());
    let bytesBig = (0, bigint_helpers_js_1.changeBase)(fieldsBigInts, p, bytesBase);
...
function toFieldsBijective(bytes, p) {
    ...
    let fieldsBigInts = (0, bigint_helpers_js_1.changeBase)(bytesBig, bytesBase, p);
    let fields = fieldsBigInts.map(wrapped_js_1.Field);
    return fields;
}
function bytesOfConstantField(field) {
    return Uint8Array.from(wrapped_js_1.Field.toBytes(field));
}
...
function bytesToBigIntArray(bytes, bytesPerBigInt) {
    ...
    for (let i = 0; i < bytes.byteLength; i += bytesPerBigInt) {
        bigints.push((0, bigint_helpers_js_1.bytesToBigInt)(bytes.subarray(i, i + bytesPerBigInt)));
    }
    return bigints;
}
